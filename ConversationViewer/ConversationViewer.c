...
{
    // Struct_ConversationViewerProc* proc =
    // (Struct_ConversationViewerProc*)ProcFind(&ProcInstruction_ConversationViewer);
...
{
    struct Struct_ConversationViewerProc * proc =
        (Struct_ConversationViewerProc *)ProcFind(&ProcInstruction_ConversationViewer);
...
static void DrawCharacterName(struct MenuProc * menu, struct MenuCommandProc * command)
{
    ...
    if (ConvoEntry->titleTextID != 0xFFFF)
    {
        ...
        string = GetStringFromIndex(0xCD); // --
...
static void DrawChapterTitle(struct MenuProc * menu, struct MenuCommandProc * command)
{
    ...
    struct ConversationViewer_Struct * ConvoEntry =
        ConvoChapterEntry[command->commandDefinitionIndex + proc->currOptionIndex];
    // ConvoEntry += ();
    u16 text_index;
...
static int SelectYes(struct MenuProc * menu, struct MenuCommandProc * command)
{
    ...
    if (ConvoEntry->scene != 0xFFFFFFFF)
    {
        ...
        if (ConvoEntry->scene)
        {
            CallMapEventEngine((void *)((int)ConvoEntry->scene), 3);
        }
        else
        {
            if (ConvoEntry->textID != 0xFFFF)
            { // if text id is 0xFFFF, do nothing
                gEventSlot[0x3] = ConvoEntry->textID;
                gEventSlot[0x2] = ConvoEntry->bg;
                u16 bgm = ConvoEntry->bgm;
                if (ConvoEntry->bg == 0xFF)
                    gEventSlot[0x2] = 0x37; // 0x37 is reserved for random BGs in vanilla
                if (bgm == 0xFFFF || bgm == 0)
                    bgm = ConversationViewer_DefaultBGM; // default bgm as Distant Roads
