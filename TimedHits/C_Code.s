...
SetCurrentAnimInProc:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	push	{r4, r5, r6, lr}	@
@ C_Code.c:138: 	proc = Proc_Find(TimedHitsProcCmd); 
	ldr	r3, .L26	@ tmp130,
@ C_Code.c:136: void SetCurrentAnimInProc(struct Anim* anim) { 
	movs	r5, r0	@ anim, tmp189
@ C_Code.c:138: 	proc = Proc_Find(TimedHitsProcCmd); 
	ldr	r0, .L26+4	@ tmp129,
	bl	.L10		@
	subs	r4, r0, #0	@ proc, tmp190,
@ C_Code.c:139: 	if (!proc) { return; } 
	beq	.L19		@,
@ C_Code.c:106: 	proc->anim2 = NULL; 
	movs	r3, #0	@ tmp131,
@ C_Code.c:120: 	proc->buttonsToPress = 0; 
...
@ C_Code.c:147: 	proc->currentRound = GetCurrentRound(proc->roundId); 
	ldr	r3, .L26+16	@ tmp150,
	bl	.L10		@
@ C_Code.c:148: 	proc->side = GetAnimPosition(anim) ^ 1; 
...
@ C_Code.c:149: 	proc->active_bunit = gpEkrBattleUnitLeft; 
	ldr	r3, .L26+24	@ tmp158,
@ C_Code.c:150: 	proc->opp_bunit = gpEkrBattleUnitRight; 
...
@ C_Code.c:151: 	if (!proc->side) { 
	cmp	r0, #1	@ _8,
	beq	.L24		@,
@ C_Code.c:155: 	if (!proc->loadedImg) {
	movs	r6, #73	@ tmp160,
@ C_Code.c:155: 	if (!proc->loadedImg) {
...
@ C_Code.c:155: 	if (!proc->loadedImg) {
	movs	r6, #73	@ tmp160,
@ C_Code.c:152: 		proc->active_bunit = gpEkrBattleUnitRight; 
	str	r2, [r4, #60]	@ gpEkrBattleUnitRight.3_12, proc_18->active_bunit
@ C_Code.c:153: 		proc->opp_bunit = gpEkrBattleUnitLeft;
...
GetButtonsToPress:
	...
	mov	r7, r9	@,
	push	{r6, r7, lr}	@
@ C_Code.c:307: 	if (AlwaysA || TimedHitsDifficultyRam->alwaysA) { return A_BUTTON; } 
	ldr	r3, .L56	@ tmp137,
@ C_Code.c:307: 	if (AlwaysA || TimedHitsDifficultyRam->alwaysA) { return A_BUTTON; } 
	ldr	r3, [r3]	@ AlwaysA, AlwaysA
@ C_Code.c:306: int GetButtonsToPress(TimedHitsProc* proc) { 
...
@ C_Code.c:307: 	if (AlwaysA || TimedHitsDifficultyRam->alwaysA) { return A_BUTTON; } 
	...
	bmi	.L45		@,
@ C_Code.c:308: 	int keys = proc->buttonsToPress;
	movs	r3, #80	@ tmp149,
@ C_Code.c:308: 	int keys = proc->buttonsToPress;
	ldrh	r0, [r0, r3]	@ <retval>,
@ C_Code.c:309: 	if (!keys) { 
	cmp	r0, #0	@ <retval>,
	bne	.L34		@,
@ C_Code.c:310: 		u8 KeysList[] = { A_BUTTON, B_BUTTON, DPAD_RIGHT, DPAD_LEFT, DPAD_UP, DPAD_DOWN }; 
	ldr	r3, .L56+8	@ tmp151,
	ldr	r1, [r3]	@ tmp154,
	str	r1, [sp]	@ tmp154,
	mov	r1, sp	@ tmp184,
	ldrh	r3, [r3, #4]	@ tmp156,
	strh	r3, [r1, #4]	@ tmp156,
@ C_Code.c:315: 		int numberOfRandomButtons = NumberOfRandomButtons; 
...
@ C_Code.c:316: 		if (!numberOfRandomButtons) { numberOfRandomButtons = TimedHitsDifficultyRam->difficulty; } 
	cmp	r3, #0	@ numberOfRandomButtons,
	beq	.L53		@,
@ C_Code.c:318: 		for (int i = 0; i < numberOfRandomButtons; ++i) { 
	ble	.L46		@,
.L37:
	ldr	r3, .L56+16	@ tmp177,
@ C_Code.c:318: 		for (int i = 0; i < numberOfRandomButtons; ++i) { 
	movs	r6, #0	@ i,
@ C_Code.c:313: 		int oppDir = 0; 
	movs	r7, #0	@ oppDir,
@ C_Code.c:308: 	int keys = proc->buttonsToPress;
	movs	r5, #0	@ keys,
@ C_Code.c:314: 		int size = 5; // -1 since we count from 0  
	movs	r4, #5	@ size,
...
@ C_Code.c:318: 		for (int i = 0; i < numberOfRandomButtons; ++i) { 
	adds	r6, r6, #1	@ i,
@ C_Code.c:338: 			keys |= button; 
	orrs	r5, r0	@ keys, button
@ C_Code.c:318: 		for (int i = 0; i < numberOfRandomButtons; ++i) { 
	cmp	r9, r6	@ numberOfRandomButtons, i
	ble	.L54		@,
.L43:
@ C_Code.c:319: 			num = NextRN_N(size); 
	movs	r0, r4	@, size
	bl	.L58		@
@ C_Code.c:320: 			button = KeysList[num];
	mov	r3, sp	@ tmp191,
	ldrb	r0, [r3, r0]	@ button, KeysList
@ C_Code.c:323: 			if (button & 0xF0) { // some dpad 
	cmp	r0, #15	@ button,
	bls	.L39		@,
@ C_Code.c:324: 				if (button == DPAD_RIGHT) { oppDir = DPAD_LEFT; } 
	cmp	r0, #16	@ button,
	beq	.L47		@,
@ C_Code.c:325: 				if (button == DPAD_LEFT) { oppDir = DPAD_RIGHT; } 
	cmp	r0, #32	@ button,
	beq	.L48		@,
@ C_Code.c:326: 				if (button == DPAD_UP) { oppDir = DPAD_DOWN; } 
	cmp	r0, #64	@ button,
	bne	.L55		@,
@ C_Code.c:326: 				if (button == DPAD_UP) { oppDir = DPAD_DOWN; } 
	movs	r7, #128	@ oppDir,
.L40:
@ C_Code.c:328: 				for (int c = 0; c <= size; ++c) { 
	cmp	r4, #0	@ size,
	blt	.L39		@,
	mov	r2, sp	@ ivtmp.90,
@ C_Code.c:328: 				for (int c = 0; c <= size; ++c) { 
...
@ C_Code.c:318: 		for (int i = 0; i < numberOfRandomButtons; ++i) { 
	...
	bgt	.L43		@,
.L54:
@ C_Code.c:340: 		proc->buttonsToPress = keys; 
...
@ C_Code.c:317: 		if (!numberOfRandomButtons) { numberOfRandomButtons = 1; }
	cmp	r3, #0	@ tmp163,
	bne	.L37		@,
@ C_Code.c:317: 		if (!numberOfRandomButtons) { numberOfRandomButtons = 1; }
...
@ C_Code.c:325: 				if (button == DPAD_LEFT) { oppDir = DPAD_RIGHT; } 
	...
	b	.L40		@
.L55:
@ C_Code.c:327: 				if (button == DPAD_DOWN) { oppDir = DPAD_UP; } 
	cmp	r0, #128	@ button,
	bne	.L40		@,
@ C_Code.c:327: 				if (button == DPAD_DOWN) { oppDir = DPAD_UP; } 
	movs	r7, #64	@ oppDir,
...
DrawButtonsToPress.part.0:
	...
	push	{r5, r6, r7, lr}	@
	sub	sp, sp, #12	@,,
@ C_Code.c:400: void DrawButtonsToPress(TimedHitsProc* proc, int x, int y, int palID) { 
	movs	r7, r2	@ y, tmp237
	movs	r4, r0	@ proc, tmp235
	mov	r9, r1	@ x, tmp236
	movs	r6, r3	@ palID, tmp238
@ C_Code.c:402: 	int keys = GetButtonsToPress(proc); 
	bl	GetButtonsToPress		@
@ C_Code.c:404: 	if (ChangePaletteWhenButtonIsPressed && proc->frame) { palID = 14; } 
	ldr	r2, .L114	@ tmp155,
@ C_Code.c:404: 	if (ChangePaletteWhenButtonIsPressed && proc->frame) { palID = 14; } 
...
@ C_Code.c:404: 	if (ChangePaletteWhenButtonIsPressed && proc->frame) { palID = 14; } 
	...
	bne	.L105		@,
.L62:
@ C_Code.c:406: 	int oam2 = OAM2_PAL(palID) | OAM2_LAYER(0); //OAM2_CHR(0);
	lsls	r6, r6, #28	@ tmp158, palID,
	lsrs	r6, r6, #16	@ _78, tmp158,
.L61:
@ C_Code.c:407: 	PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_PressInput, oam2); 
	mov	r2, r9	@ x, x
	movs	r4, #255	@ tmp161,
...
@ C_Code.c:408: 	x += 32; 
	mov	r1, r9	@ x, x
@ C_Code.c:409: 	PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_PressInput2, oam2); 
...
@ C_Code.c:409: 	PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_PressInput2, oam2); 
	...
	str	r6, [sp]	@ _78,
	bl	.L116		@
	mov	r3, r8	@ ivtmp.104, tmp230
	mov	r1, r8	@ _76, tmp230
@ C_Code.c:407: 	PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_PressInput, oam2); 
	mov	r10, r4	@ tmp231, tmp231
@ C_Code.c:347: 	int c = 0; 
	movs	r2, #0	@ c,
@ C_Code.c:410: 	y += 16; x -= 36; 
	adds	r7, r7, #16	@ y,
	adds	r3, r3, #48	@ ivtmp.104,
	adds	r1, r1, #54	@ _76,
.L64:
@ C_Code.c:349: 		if (keys & RomKeysList[i]) { c++; } 
...
@ C_Code.c:348: 	for (int i = 0; i < 6; ++i) { 
	adds	r3, r3, #1	@ ivtmp.104,
@ C_Code.c:349: 		if (keys & RomKeysList[i]) { c++; } 
	adds	r2, r2, r4	@ c, c, tmp233
@ C_Code.c:348: 	for (int i = 0; i < 6; ++i) { 
	cmp	r1, r3	@ _76, ivtmp.104
	bne	.L64		@,
@ C_Code.c:413: 	if (count == 1) { x += 16; } // centering 
	cmp	r2, #1	@ c,
	beq	.L106		@,
@ C_Code.c:414: 	if (count == 2) { x += 8; } 
	mov	r3, r9	@ x, x
	adds	r4, r3, #4	@ x, x,
@ C_Code.c:414: 	if (count == 2) { x += 8; } 
	cmp	r2, #2	@ c,
	bne	.L107		@,
.L66:
@ C_Code.c:417: 	if (keys & A_BUTTON) { 
	lsls	r3, r5, #31	@ tmp241, keys,
	bmi	.L108		@,
.L68:
@ C_Code.c:420: 	if (keys & B_BUTTON) { 
	lsls	r3, r5, #30	@ tmp242, keys,
	bmi	.L109		@,
.L69:
@ C_Code.c:423: 	if (keys & DPAD_LEFT) { 
	lsls	r3, r5, #26	@ tmp243, keys,
	bmi	.L110		@,
.L70:
@ C_Code.c:426: 	if (keys & DPAD_RIGHT) { 
	lsls	r3, r5, #27	@ tmp244, keys,
	bmi	.L111		@,
.L71:
@ C_Code.c:429: 	if (keys & DPAD_UP) { 
	lsls	r3, r5, #25	@ tmp245, keys,
	bmi	.L112		@,
.L72:
@ C_Code.c:432: 	if (keys & DPAD_DOWN) { 
...
@ C_Code.c:417: 	if (keys & A_BUTTON) { 
	...
	bpl	.L68		@,
.L108:
@ C_Code.c:418: 		PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_A_Button, oam2); x += 18; 
...
@ C_Code.c:420: 	if (keys & B_BUTTON) { 
	...
	bpl	.L69		@,
.L109:
@ C_Code.c:421: 		PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_B_Button, oam2); x += 18; 
...
@ C_Code.c:423: 	if (keys & DPAD_LEFT) { 
	...
	bpl	.L70		@,
.L110:
@ C_Code.c:424: 		PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_Left_Button, oam2); x += 18; 
...
@ C_Code.c:426: 	if (keys & DPAD_RIGHT) { 
	...
	bpl	.L71		@,
.L111:
@ C_Code.c:427: 		PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_Right_Button, oam2); x += 18; 
...
@ C_Code.c:429: 	if (keys & DPAD_UP) { 
	...
	bpl	.L72		@,
.L112:
@ C_Code.c:430: 		PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_Up_Button, oam2); x += 18; 
...
@ C_Code.c:432: 	if (keys & DPAD_DOWN) { 
	...
	bpl	.L59		@,
.L113:
@ C_Code.c:433: 		PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_Down_Button, oam2); x += 18; 
...
CountKeysPressed:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	ldr	r1, .L123	@ tmp122,
	movs	r2, r1	@ ivtmp.115, tmp122
	push	{r4, r5, lr}	@
@ C_Code.c:346: int CountKeysPressed(u32 keys) { 
	movs	r4, r0	@ keys, tmp129
@ C_Code.c:347: 	int c = 0; 
	movs	r0, #0	@ <retval>,
	adds	r2, r2, #48	@ ivtmp.115,
	adds	r1, r1, #54	@ _15,
.L119:
@ C_Code.c:349: 		if (keys & RomKeysList[i]) { c++; } 
...
@ C_Code.c:348: 	for (int i = 0; i < 6; ++i) { 
	adds	r2, r2, #1	@ ivtmp.115,
@ C_Code.c:349: 		if (keys & RomKeysList[i]) { c++; } 
	adds	r0, r0, r3	@ <retval>, <retval>, tmp127
@ C_Code.c:348: 	for (int i = 0; i < 6; ++i) { 
	cmp	r2, r1	@ ivtmp.115, _15
	bne	.L119		@,
@ C_Code.c:353: } 
	@ sp needed	@
	pop	{r4, r5}
...
@ C_Code.c:355: int PressedSpecificKeys(TimedHitsProc* proc, u32 keys) { 
	movs	r4, r1	@ keys, tmp180
@ C_Code.c:356: 	int reqKeys = GetButtonsToPress(proc); 
	bl	GetButtonsToPress		@
	ldr	r5, .L151	@ tmp150,
	movs	r2, r5	@ ivtmp.138, tmp150
	adds	r2, r2, #48	@ ivtmp.138,
	mov	ip, r0	@ reqKeys, tmp181
@ C_Code.c:357: 	int count = CountKeysPressed(reqKeys); 
	movs	r1, r2	@ ivtmp.150, ivtmp.138
@ C_Code.c:347: 	int c = 0; 
	movs	r6, #0	@ c,
	adds	r5, r5, #54	@ _58,
.L127:
...
@ C_Code.c:348: 	for (int i = 0; i < 6; ++i) { 
	adds	r1, r1, #1	@ ivtmp.150,
@ C_Code.c:349: 		if (keys & RomKeysList[i]) { c++; } 
	adds	r6, r6, r3	@ c, c, tmp168
@ C_Code.c:348: 	for (int i = 0; i < 6; ++i) { 
	cmp	r5, r1	@ _58, ivtmp.150
	bne	.L127		@,
	movs	r1, r2	@ ivtmp.144, ivtmp.138
@ C_Code.c:347: 	int c = 0; 
	movs	r7, #0	@ c,
.L129:
@ C_Code.c:349: 		if (keys & RomKeysList[i]) { c++; } 
...
@ C_Code.c:348: 	for (int i = 0; i < 6; ++i) { 
	adds	r1, r1, #1	@ ivtmp.144,
@ C_Code.c:349: 		if (keys & RomKeysList[i]) { c++; } 
	adds	r7, r7, r3	@ c, c, tmp171
@ C_Code.c:348: 	for (int i = 0; i < 6; ++i) { 
	cmp	r5, r1	@ _58, ivtmp.144
	bne	.L129		@,
@ C_Code.c:347: 	int c = 0; 
	movs	r1, #0	@ c,
@ C_Code.c:358: 	if (ABS(count - CountKeysPressed(keys)) > 1) { return false; } // you pressed more than 1 extra key. Shame on you. 
	cmp	r6, r7	@ c, c
	blt	.L130		@,
.L132:
@ C_Code.c:349: 		if (keys & RomKeysList[i]) { c++; } 
...
@ C_Code.c:348: 	for (int i = 0; i < 6; ++i) { 
	adds	r2, r2, #1	@ ivtmp.138,
@ C_Code.c:349: 		if (keys & RomKeysList[i]) { c++; } 
	adds	r1, r1, r3	@ c, c, tmp174
@ C_Code.c:348: 	for (int i = 0; i < 6; ++i) { 
	cmp	r2, r5	@ ivtmp.138, _58
	bne	.L132		@,
@ C_Code.c:358: 	if (ABS(count - CountKeysPressed(keys)) > 1) { return false; } // you pressed more than 1 extra key. Shame on you. 
...
@ C_Code.c:358: 	if (ABS(count - CountKeysPressed(keys)) > 1) { return false; } // you pressed more than 1 extra key. Shame on you. 
	...
	bgt	.L138		@,
.L134:
@ C_Code.c:359: 	reqKeys &= ~keys; // only 0 if we hit all the correct keys (and possibly 1 extra) 
	mov	r0, ip	@ reqKeys, reqKeys
	bics	r0, r4	@ reqKeys, keys
@ C_Code.c:360: 	return (!reqKeys); 
...
SaveInputFrame:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	push	{r4, r5, r6, lr}	@
@ C_Code.c:363: 	struct Anim* anim = proc->anim; 
	ldr	r5, [r0, #44]	@ anim, proc_17(D)->anim
@ C_Code.c:364: 	u32 instruction = *anim->pScrCurrent++; 
	ldr	r6, [r5, #32]	@ _1, anim_18->pScrCurrent
@ C_Code.c:364: 	u32 instruction = *anim->pScrCurrent++; 
	adds	r3, r6, #4	@ tmp130, _1,
	str	r3, [r5, #32]	@ tmp130, anim_18->pScrCurrent
@ C_Code.c:365: 	if (ANINS_GET_TYPE(instruction) == ANIM_INS_TYPE_COMMAND) {
	movs	r3, #252	@ tmp132,
@ C_Code.c:362: void SaveInputFrame(TimedHitsProc* proc, u32 keys) { 
...
@ C_Code.c:365: 	if (ANINS_GET_TYPE(instruction) == ANIM_INS_TYPE_COMMAND) {
	...
	beq	.L160		@,
.L154:
@ C_Code.c:374: 	if (PressedSpecificKeys(proc, keys)) { 
	movs	r0, r4	@, proc
@ C_Code.c:373: 	instruction = *anim->pScrCurrent--; 
...
@ C_Code.c:380: }  
	...
	bx	r0
.L160:
@ C_Code.c:366: 		if (ANINS_COMMAND_GET_ID(instruction) == 4) {
	movs	r3, #255	@ tmp134,
	ands	r3, r2	@ _4, instruction
@ C_Code.c:366: 		if (ANINS_COMMAND_GET_ID(instruction) == 4) {
	cmp	r3, #4	@ _4,
	beq	.L162		@,
@ C_Code.c:369: 		if (ANINS_COMMAND_GET_ID(instruction) == 0xF) {
	cmp	r3, #15	@ _4,
	bne	.L154		@,
@ C_Code.c:370: 			proc->codefframe = proc->timer; proc->timer2 = 0; 
...
@ C_Code.c:370: 			proc->codefframe = proc->timer; proc->timer2 = 0; 
	...
	b	.L154		@
.L161:
@ C_Code.c:377: 			PlaySFX(0x13e, 0x100, 120, 1); //PlaySFX(int songid, int volume, int locate, int type)
	movs	r1, #128	@,
	movs	r0, #159	@,
@ C_Code.c:376: 			proc->frame = proc->timer; // locate is side for stereo? 
	ldr	r2, [r4, #52]	@ proc_17(D)->timer, proc_17(D)->timer
@ C_Code.c:377: 			PlaySFX(0x13e, 0x100, 120, 1); //PlaySFX(int songid, int volume, int locate, int type)
	lsls	r1, r1, #1	@,,
@ C_Code.c:376: 			proc->frame = proc->timer; // locate is side for stereo? 
...
@ C_Code.c:380: }  
	b	.L153		@
.L162:
@ C_Code.c:367: 			proc->code4frame = proc->timer; proc->timer2 = 0; 
	ldr	r2, [r4, #52]	@ proc_17(D)->timer, proc_17(D)->timer
...
@ C_Code.c:382: 	if (proc->frame) { 
	movs	r3, #75	@ tmp128,
	ldrb	r3, [r0, r3]	@ _1,
@ C_Code.c:381: void SaveIfWeHitOnTime(TimedHitsProc* proc) {
	push	{r4, lr}	@
@ C_Code.c:382: 	if (proc->frame) { 
	cmp	r3, #0	@ _1,
	beq	.L165		@,
@ C_Code.c:383: 		if (proc->codefframe != 0xFF) { if (ABS(proc->codefframe - proc->frame) < (LenienceFrames)) { proc->hitOnTime = true; } }
	movs	r2, #77	@ tmp129,
@ C_Code.c:383: 		if (proc->codefframe != 0xFF) { if (ABS(proc->codefframe - proc->frame) < (LenienceFrames)) { proc->hitOnTime = true; } }
	ldr	r1, .L176	@ tmp130,
@ C_Code.c:383: 		if (proc->codefframe != 0xFF) { if (ABS(proc->codefframe - proc->frame) < (LenienceFrames)) { proc->hitOnTime = true; } }
...
.L169:
@ C_Code.c:385: 		if ((proc->timer - proc->frame) < LenienceFrames) { proc->hitOnTime = true; } 
...
@ C_Code.c:388: }
	...
	bx	r0
.L168:
@ C_Code.c:384: 		else if (proc->code4frame != 0xFF) { if (ABS(proc->code4frame - proc->frame) < (LenienceFrames)) { proc->hitOnTime = true; } } 
	movs	r2, #76	@ tmp136,
	ldrb	r2, [r0, r2]	@ _8,
@ C_Code.c:384: 		else if (proc->code4frame != 0xFF) { if (ABS(proc->code4frame - proc->frame) < (LenienceFrames)) { proc->hitOnTime = true; } } 
	cmp	r2, #255	@ _8,
...
CheatCodeOn:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	@ link register save eliminated.
@ C_Code.c:391: 	return (gPlaySt.unk1F & 0x80); // 202BD0F
	ldr	r3, .L179	@ tmp118,
@ C_Code.c:392: } 
	@ sp needed	@
@ C_Code.c:391: 	return (gPlaySt.unk1F & 0x80); // 202BD0F
...
@ C_Code.c:391: 	return (gPlaySt.unk1F & 0x80); // 202BD0F
	ldr	r3, .L185	@ tmp120,
@ C_Code.c:395: 	if (CheatCodeOn()) { return true; } 
	ldrb	r3, [r3, #31]	@ tmp123,
	cmp	r3, #127	@ tmp123,
	bhi	.L184		@,
@ C_Code.c:396: 	if (AlwaysWork) { return true; } 
	ldr	r3, .L185+4	@ tmp124,
@ C_Code.c:396: 	if (AlwaysWork) { return true; } 
	ldr	r3, [r3]	@ AlwaysWork, AlwaysWork
	cmp	r3, #0	@ AlwaysWork,
	bne	.L184		@,
@ C_Code.c:397: 	return proc->hitOnTime;
...
DrawButtonsToPress:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	push	{r4, lr}	@
@ C_Code.c:401: 	if (!DisplayPress) { return; } 
	ldr	r4, .L192	@ tmp119,
@ C_Code.c:401: 	if (!DisplayPress) { return; } 
...
@ C_Code.c:522: 	if (success) { 
	cmp	r2, #0	@ tmp131,
	beq	.L195		@,
@ C_Code.c:523: 		if (UNIT_FACTION(&active_bunit->unit) == FACTION_RED) { 
	cmp	r3, #128	@ _14,
	bne	.L196		@,
@ C_Code.c:524: 			if (BlockingEnabled) { return ReducedDamagePercent; }
	ldr	r3, .L200	@ tmp124,
@ C_Code.c:524: 			if (BlockingEnabled) { return ReducedDamagePercent; }
...
@ C_Code.c:529: 	if (UNIT_FACTION(&active_bunit->unit) == FACTION_RED) { 
	cmp	r3, #128	@ _14,
	beq	.L199		@,
@ C_Code.c:532: 	return FailedHitDamagePercent; 
...
@ C_Code.c:533: } 
	...
	bx	lr
.L196:
@ C_Code.c:527: 		return BonusDamagePercent; 
	ldr	r3, .L200+12	@ tmp127,
	ldr	r0, [r3]	@ <retval>,
	b	.L194		@
.L199:
@ C_Code.c:525: 			else { return 100; } 
	movs	r0, #100	@ <retval>,
	b	.L194		@
.L201:
	.align	2
...
@ C_Code.c:525: 			else { return 100; } 
	movs	r0, #100	@ <retval>,
@ C_Code.c:536: 	return GetDefaultDamagePercent(active_bunit, opp_bunit, success); 
	b	.L202		@
.L209:
	.align	2
.L208:
...
AdjustCurrentRound:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	push	{r4, r5, lr}	@
@ C_Code.c:552: 	for (int i = id; i < 22; i += 2) {
	cmp	r0, #21	@ id,
	bgt	.L210		@,
	ldr	r3, .L220	@ tmp128,
	lsls	r2, r0, #1	@ tmp127, id,
@ C_Code.c:554: 		if (hp == 0xffff) { break; }
	ldr	r5, .L220+4	@ tmp129,
	adds	r2, r2, r3	@ ivtmp.180, tmp127, tmp128
	b	.L214		@
.L212:
	movs	r4, #0	@ _4,
@ C_Code.c:556: 		else if (hp >= difference) { gEfxHpLut[i] -= difference; }
...
@ C_Code.c:552: 	for (int i = id; i < 22; i += 2) {
	adds	r0, r0, #2	@ id,
@ C_Code.c:555: 		if (difference < 0) { hp += difference; if (hp > 0) { gEfxHpLut[i] = hp; } else { gEfxHpLut[i] = 0; } }
...
UpdateHP:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	push	{r4, r5, r6, r7, lr}	@
@ C_Code.c:564: 	if (newHp < 0) { newHp = 0; } 
	mvns	r4, r3	@ tmp149, newHp
@ C_Code.c:565: 	int hp = gEkrGaugeHp[side];
...
@ C_Code.c:566: 	some_bunit->unit.curHP = newHp; 
	strb	r3, [r2, #19]	@ _13, some_bunit_22(D)->unit.curHP
@ C_Code.c:567: 	if (hp == newHp) { return; } 
	cmp	r3, r4	@ _13, _1
	beq	.L222		@,
@ C_Code.c:569: 	if (newDamage) { diff = 0 - newDamage; }
	ldr	r2, [sp, #24]	@ tmp162, newDamage
	rsbs	r5, r2, #0	@ diff, tmp162
@ C_Code.c:569: 	if (newDamage) { diff = 0 - newDamage; }
	cmp	r2, #0	@ tmp163,
	beq	.L231		@,
@ C_Code.c:571: 	if (proc->side == side) { 
	movs	r2, #74	@ tmp137,
@ C_Code.c:571: 	if (proc->side == side) { 
...
@ C_Code.c:571: 	if (proc->side == side) { 
	ldrb	r2, [r0, r2]	@ tmp138,
@ C_Code.c:568: 	int diff = newHp - hp; 
	subs	r5, r3, r4	@ diff, _13, _1
@ C_Code.c:571: 	if (proc->side == side) { 
	cmp	r2, r6	@ tmp138, tmp164
	bne	.L222		@,
.L232:
@ C_Code.c:573: 		if (UsingSkillSys) { // uggggh 
	ldr	r2, .L233+4	@ tmp140,
@ C_Code.c:582: 		proc->currentRound->hpChange = ABS(diff); 
...
@ C_Code.c:583: 		if (UsingSkillSys == 2) { proc->currentRound->overDmg = diff; } // used by Huichelaar's banim numbers 
...
@ C_Code.c:590: 	int side = proc->side; 
	movs	r3, #74	@ tmp164,
@ C_Code.c:589: void CheckForDeath(TimedHitsProc* proc, struct NewProcEfxHPBar* HpProc, struct BattleUnit* active_bunit, struct BattleUnit* opp_bunit, struct SkillSysBattleHit* round, int hp, int newDamage) { 
	push	{lr}	@
	sub	sp, sp, #8	@,,
@ C_Code.c:589: void CheckForDeath(TimedHitsProc* proc, struct NewProcEfxHPBar* HpProc, struct BattleUnit* active_bunit, struct BattleUnit* opp_bunit, struct SkillSysBattleHit* round, int hp, int newDamage) { 
...
@ C_Code.c:590: 	int side = proc->side; 
	mov	r8, r3	@ _1, _1
@ C_Code.c:592: 	if (hp < 0) { hp = gEkrGaugeHp[side]; } 
	cmp	r1, #0	@ hp,
	bge	.L236		@,
@ C_Code.c:592: 	if (hp < 0) { hp = gEkrGaugeHp[side]; } 
	mov	r1, r8	@ _1, _1
...
.L236:
@ C_Code.c:593: 	if (hp <= 0) { // they are dead 
	cmp	r1, #0	@ hp,
	ble	.L251		@,
@ C_Code.c:638: 		HpProc->death = false; 
	movs	r3, #41	@ tmp225,
	movs	r2, #0	@ tmp226,
	strb	r2, [r6, r3]	@ tmp226, HpProc_31(D)->death
.L244:
@ C_Code.c:643: 	BattleApplyExpGains();  // update exp 
	ldr	r3, .L254+4	@ tmp228,
	bl	.L10		@
@ C_Code.c:644: 	gBanimExpGain[0] = gpEkrBattleUnitLeft->expGain; 
...
@ C_Code.c:645: 	gBanimExpGain[1] = gpEkrBattleUnitRight->expGain; 
...
@ C_Code.c:647: }
	...
	bx	r0
.L251:
@ C_Code.c:595: 		UpdateHP(proc, HpProc, opp_bunit, hp, side, newDamage); 
	...
	movs	r3, #0	@,
	bl	UpdateHP		@
@ C_Code.c:597: 		proc->code4frame = 0xff;
	movs	r3, #76	@ tmp167,
	movs	r2, #255	@ tmp168,
@ C_Code.c:615: 		struct SkillSysBattleHit* nextRound = GetCurrentRound(proc->roundId + 1); 
	movs	r4, #70	@ tmp180,
@ C_Code.c:597: 		proc->code4frame = 0xff;
	strb	r2, [r5, r3]	@ tmp168, proc_27(D)->code4frame
@ C_Code.c:602: 		HpProc->death = true; 
	subs	r3, r3, #35	@ tmp170,
	subs	r2, r2, #254	@ tmp171,
	strb	r2, [r6, r3]	@ tmp171, HpProc_31(D)->death
@ C_Code.c:613: 		round->info |= BATTLE_HIT_INFO_FINISHES | BATTLE_HIT_INFO_KILLS_TARGET | BATTLE_HIT_INFO_END; 
...
@ C_Code.c:615: 		struct SkillSysBattleHit* nextRound = GetCurrentRound(proc->roundId + 1); 
	...
	adds	r0, r0, #1	@ tmp182,
	bl	.L10		@
@ C_Code.c:616: 		nextRound->info = BATTLE_HIT_INFO_END; 
	movs	r3, #7	@ tmp189,
...
@ C_Code.c:620: 		u16* animRound = &GetAnimRoundData()[0]; 
	ldr	r3, .L254+24	@ tmp195,
	bl	.L10		@
@ C_Code.c:621: 		for (int i = proc->roundId; i < 32; ++i) { 
	ldrb	r3, [r5, r4]	@ i,
@ C_Code.c:621: 		for (int i = proc->roundId; i < 32; ++i) { 
	cmp	r3, #31	@ i,
	bgt	.L241		@,
@ C_Code.c:623: 			animRound[i] = 0xFFFF; 
	movs	r2, #1	@ tmp245,
	rsbs	r2, r2, #0	@ tmp245, tmp245
	mov	ip, r2	@ tmp245, tmp245
@ C_Code.c:622: 			if (animRound[i] == 0xFFFF) { break; } 
...
@ C_Code.c:632: 		if (round->attributes & BATTLE_HIT_ATTR_HPSTEAL) { hp += newDamage; } 
...
@ C_Code.c:651: 	if ((proc->currentRound->attributes & BATTLE_HIT_ATTR_MISS) || (!proc->currentRound->hpChange)) { return; } 
	ldr	r3, [r0, #56]	@ _1, proc_37(D)->currentRound
	mov	r8, r3	@ _1, _1
@ C_Code.c:651: 	if ((proc->currentRound->attributes & BATTLE_HIT_ATTR_MISS) || (!proc->currentRound->hpChange)) { return; } 
	ldr	r3, [r3]	@ *_1, *_1
@ C_Code.c:649: void AdjustDamageByPercent(TimedHitsProc* proc, struct NewProcEfxHPBar* HpProc, struct BattleUnit* active_bunit, struct BattleUnit* opp_bunit, struct SkillSysBattleHit* round, int percent) { 
...
@ C_Code.c:651: 	if ((proc->currentRound->attributes & BATTLE_HIT_ATTR_MISS) || (!proc->currentRound->hpChange)) { return; } 
	...
	beq	.L256		@,
@ C_Code.c:652: 	if (round->hpChange <= 0) { return; } // healing 
	movs	r1, #3	@ _5,
...
@ C_Code.c:652: 	if (round->hpChange <= 0) { return; } // healing 
	cmp	r1, #0	@ _5,
	ble	.L256		@,
@ C_Code.c:653: 	int side = proc->side; 
	movs	r3, #74	@ tmp160,
	ldrb	r3, [r0, r3]	@ side,
	movs	r2, r3	@ side, side
	str	r3, [sp, #16]	@ side, %sfp
@ C_Code.c:654: 	int hp = gEkrGaugeHp[proc->side];
	ldr	r3, .L289	@ tmp161,
	lsls	r2, r2, #1	@ tmp162, side,
@ C_Code.c:654: 	int hp = gEkrGaugeHp[proc->side];
	ldrsh	r3, [r2, r3]	@ hp, gEkrGaugeHp
	str	r3, [sp, #20]	@ hp, %sfp
@ C_Code.c:655: 	if (!hp) { CheckForDeath(proc, HpProc, active_bunit, opp_bunit, round, hp, 0); return; } 
	cmp	r3, #0	@ hp,
	bne	.LCB1564	@
	b	.L283	@long jump	@
.LCB1564:
@ C_Code.c:658: 	if (gEkrGaugeDmg[side ^ 1] > oldDamage) { oldDamage = gEkrGaugeDmg[side ^ 1]; } 
	movs	r3, #1	@ tmp170,
	ldr	r0, [sp, #16]	@ side, %sfp
@ C_Code.c:658: 	if (gEkrGaugeDmg[side ^ 1] > oldDamage) { oldDamage = gEkrGaugeDmg[side ^ 1]; } 
	ldr	r2, .L289+4	@ tmp165,
@ C_Code.c:658: 	if (gEkrGaugeDmg[side ^ 1] > oldDamage) { oldDamage = gEkrGaugeDmg[side ^ 1]; } 
	eors	r3, r0	@ tmp169, side
@ C_Code.c:658: 	if (gEkrGaugeDmg[side ^ 1] > oldDamage) { oldDamage = gEkrGaugeDmg[side ^ 1]; } 
	lsls	r3, r3, #1	@ tmp171, tmp169,
@ C_Code.c:658: 	if (gEkrGaugeDmg[side ^ 1] > oldDamage) { oldDamage = gEkrGaugeDmg[side ^ 1]; } 
...
@ C_Code.c:658: 	if (gEkrGaugeDmg[side ^ 1] > oldDamage) { oldDamage = gEkrGaugeDmg[side ^ 1]; } 
	...
	ble	.L284		@,
@ C_Code.c:659: 	if (UsingSkillSys == 2) { oldDamage = ABS(round->overDmg); } 
	ldr	r3, .L289+8	@ tmp172,
	ldr	r3, [r3]	@ UsingSkillSys.31_10, UsingSkillSys
	str	r3, [sp, #24]	@ UsingSkillSys.31_10, %sfp
@ C_Code.c:659: 	if (UsingSkillSys == 2) { oldDamage = ABS(round->overDmg); } 
	cmp	r3, #2	@ UsingSkillSys.31_10,
	beq	.L285		@,
.L261:
@ C_Code.c:663: 	int newDamage = ((oldDamage * percent)) / 100; 
	ldr	r2, [sp, #76]	@ tmp231, percent
	mov	r3, r10	@ _13, oldDamage
	muls	r3, r2	@ _13, tmp231
@ C_Code.c:663: 	int newDamage = ((oldDamage * percent)) / 100; 
	ldr	r2, .L289+12	@ tmp178,
	movs	r1, #100	@,
	movs	r0, r3	@, _13
	str	r3, [sp, #28]	@ _13, %sfp
	mov	fp, r2	@ tmp178, tmp178
	bl	.L291		@
	mov	r9, r0	@ newDamage, tmp203
@ C_Code.c:664: 	if (newDamage >= oldDamage) { newDamage = ((oldDamage * percent) + BonusDamageRounding) / 100; } 
	cmp	r10, r0	@ oldDamage, newDamage
	bgt	.L262		@,
@ C_Code.c:664: 	if (newDamage >= oldDamage) { newDamage = ((oldDamage * percent) + BonusDamageRounding) / 100; } 
	ldr	r3, .L289+16	@ tmp180,
	ldr	r0, [r3]	@ BonusDamageRounding, BonusDamageRounding
	ldr	r3, [sp, #28]	@ _13, %sfp
	mov	ip, r3	@ _13, _13
@ C_Code.c:664: 	if (newDamage >= oldDamage) { newDamage = ((oldDamage * percent) + BonusDamageRounding) / 100; } 
	movs	r1, #100	@,
@ C_Code.c:664: 	if (newDamage >= oldDamage) { newDamage = ((oldDamage * percent) + BonusDamageRounding) / 100; } 
	add	r0, r0, ip	@ tmp181, _13
@ C_Code.c:664: 	if (newDamage >= oldDamage) { newDamage = ((oldDamage * percent) + BonusDamageRounding) / 100; } 
	bl	.L292		@
	mov	r9, r0	@ newDamage, tmp204
.L262:
@ C_Code.c:665: 	if (!newDamage) { newDamage = 1; } 
...
.L263:
@ C_Code.c:666: 	int newHp = hp - newDamage; 
	mov	r2, r9	@ newDamage, newDamage
	ldr	r3, [sp, #20]	@ hp, %sfp
@ C_Code.c:667: 	if (UNIT_FACTION(&active_bunit->unit) == FACTION_RED) { 
	movs	r1, #11	@ tmp187,
@ C_Code.c:666: 	int newHp = hp - newDamage; 
...
@ C_Code.c:667: 	if (UNIT_FACTION(&active_bunit->unit) == FACTION_RED) { 
	...
	beq	.L286		@,
.L264:
@ C_Code.c:674: 	if (newHp <= 0) { newHp = 0; } 
	mvns	r3, r2	@ tmp198, newHp
	asrs	r3, r3, #31	@ tmp197, tmp198,
	ands	r2, r3	@ newHp, tmp197
@ C_Code.c:677: 	if (UsingSkillSys && (!ProcSkillsStackWithTimedHits) && (proc->currentRound->skillID)) { 
	ldr	r3, [sp, #24]	@ UsingSkillSys.31_10, %sfp
@ C_Code.c:674: 	if (newHp <= 0) { newHp = 0; } 
	mov	fp, r2	@ _3, newHp
@ C_Code.c:677: 	if (UsingSkillSys && (!ProcSkillsStackWithTimedHits) && (proc->currentRound->skillID)) { 
...
@ C_Code.c:677: 	if (UsingSkillSys && (!ProcSkillsStackWithTimedHits) && (proc->currentRound->skillID)) { 
	...
	bne	.L287		@,
.L267:
@ C_Code.c:682: 	else { UpdateHP(proc, HpProc, opp_bunit, newHp, side, newDamage); } 
	mov	r3, r9	@ newDamage, newDamage
	...
	bl	UpdateHP		@
.L268:
@ C_Code.c:686: 	CheckForDeath(proc, HpProc, active_bunit, opp_bunit, round, newHp, newDamage); 
	mov	r3, r9	@ newDamage, newDamage
...
@ C_Code.c:659: 	if (UsingSkillSys == 2) { oldDamage = ABS(round->overDmg); } 
	ldr	r3, .L289+8	@ tmp172,
	ldr	r3, [r3]	@ UsingSkillSys.31_10, UsingSkillSys
@ C_Code.c:657: 	int oldDamage = round->hpChange;  
...
@ C_Code.c:659: 	if (UsingSkillSys == 2) { oldDamage = ABS(round->overDmg); } 
	...
	b	.L285		@
.L286:
@ C_Code.c:670: 			if ((hp - oldDamage) <= 0) { if (!BlockingCanPreventLethal) { newHp = hp - oldDamage; newDamage = oldDamage; } }
	mov	r1, r10	@ oldDamage, oldDamage
	ldr	r3, [sp, #20]	@ hp, %sfp
	subs	r3, r3, r1	@ _19, hp, oldDamage
@ C_Code.c:670: 			if ((hp - oldDamage) <= 0) { if (!BlockingCanPreventLethal) { newHp = hp - oldDamage; newDamage = oldDamage; } }
	cmp	r3, #0	@ _19,
	ble	.L288		@,
.L265:
@ C_Code.c:671: 			if (!BlockingEnabled) { newHp = hp - oldDamage; newDamage = oldDamage; } 
	ldr	r1, .L289+24	@ tmp192,
@ C_Code.c:671: 			if (!BlockingEnabled) { newHp = hp - oldDamage; newDamage = oldDamage; } 
	ldr	r1, [r1]	@ BlockingEnabled, BlockingEnabled
	cmp	r1, #0	@ BlockingEnabled,
	bne	.L264		@,
@ C_Code.c:671: 			if (!BlockingEnabled) { newHp = hp - oldDamage; newDamage = oldDamage; } 
	movs	r2, r3	@ newHp, _19
@ C_Code.c:671: 			if (!BlockingEnabled) { newHp = hp - oldDamage; newDamage = oldDamage; } 
...
@ C_Code.c:655: 	if (!hp) { CheckForDeath(proc, HpProc, active_bunit, opp_bunit, round, hp, 0); return; } 
	...
	b	.L282		@
.L287:
@ C_Code.c:680: 		newHp = hp - oldDamage; 
	mov	r2, r10	@ oldDamage, oldDamage
	ldr	r3, [sp, #20]	@ hp, %sfp
	subs	r3, r3, r2	@ _3, hp, oldDamage
	mov	fp, r3	@ _3, _3
@ C_Code.c:679: 		newDamage = oldDamage; 
	mov	r9, r10	@ newDamage, oldDamage
@ C_Code.c:680: 		newHp = hp - oldDamage; 
	b	.L268		@
.L288:
@ C_Code.c:670: 			if ((hp - oldDamage) <= 0) { if (!BlockingCanPreventLethal) { newHp = hp - oldDamage; newDamage = oldDamage; } }
	ldr	r1, .L289+28	@ tmp190,
@ C_Code.c:670: 			if ((hp - oldDamage) <= 0) { if (!BlockingCanPreventLethal) { newHp = hp - oldDamage; newDamage = oldDamage; } }
...
@ C_Code.c:541: void AdjustDamageWithGetter(TimedHitsProc* proc, struct NewProcEfxHPBar* HpProc, struct BattleUnit* active_bunit, struct BattleUnit* opp_bunit, struct SkillSysBattleHit* round, int success) { 
	sub	sp, sp, #12	@,,
@ C_Code.c:523: 		if (UNIT_FACTION(&active_bunit->unit) == FACTION_RED) { 
...
@ C_Code.c:524: 			if (BlockingEnabled) { return ReducedDamagePercent; }
	...
	beq	.L293		@,
@ C_Code.c:524: 			if (BlockingEnabled) { return ReducedDamagePercent; }
	ldr	r4, .L308+4	@ tmp129,
	ldr	r4, [r4]	@ _19,
@ C_Code.c:543: 	if (percent != 100) { 
	cmp	r4, #100	@ _19,
	bne	.L307		@,
.L293:
@ C_Code.c:546: }
...
@ C_Code.c:543: 	if (percent != 100) { 
	...
	beq	.L293		@,
.L307:
@ C_Code.c:544: 		AdjustDamageByPercent(proc, HpProc, active_bunit, opp_bunit, round, percent);
	str	r4, [sp, #4]	@ _19,
...
@ C_Code.c:97: 	if (gBattleStats.config & (BATTLE_CONFIG_PROMOTION | BATTLE_CONFIG_ARENA | BATTLE_CONFIG_REFRESH | BATTLE_CONFIG_MAPANIMS | BATTLE_CONFIG_PROMOTION_PREP | BATTLE_CONFIG_DANCERING)) { 
	...
	movs	r0, #252	@ tmp180,
@ C_Code.c:442: void DoStuffIfHit(TimedHitsProc* proc, struct ProcEkrBattle* battleProc, struct NewProcEfxHPBar* HpProc, struct SkillSysBattleHit* round) { 
	push	{r6, r7, lr}	@
@ C_Code.c:97: 	if (gBattleStats.config & (BATTLE_CONFIG_PROMOTION | BATTLE_CONFIG_ARENA | BATTLE_CONFIG_REFRESH | BATTLE_CONFIG_MAPANIMS | BATTLE_CONFIG_PROMOTION_PREP | BATTLE_CONFIG_DANCERING)) { 
...
.L358:
@ C_Code.c:101: 	return !CheckFlag(DisabledFlag); 
	ldr	r3, .L362+8	@ tmp195,
	ldr	r0, [r3]	@ DisabledFlag, DisabledFlag
	ldr	r3, .L362+12	@ tmp197,
	bl	.L10		@
@ C_Code.c:443: 	if (!AreTimedHitsEnabled()) { return; } 
	cmp	r0, #0	@ tmp303,
	bne	.L310		@,
@ C_Code.c:444: 	if (round->hpChange < 0) { return; } // healing 
	ldrb	r3, [r6, #3]	@ tmp202,
	cmp	r3, #127	@ tmp202,
	bhi	.L310		@,
@ C_Code.c:445: 	u32 keys = sKeyStatusBuffer.newKeys | sKeyStatusBuffer.heldKeys; 
	ldr	r3, .L362+16	@ tmp203,
@ C_Code.c:445: 	u32 keys = sKeyStatusBuffer.newKeys | sKeyStatusBuffer.heldKeys; 
	ldrh	r2, [r3, #8]	@ tmp206,
	ldrh	r3, [r3, #4]	@ tmp208,
@ C_Code.c:445: 	u32 keys = sKeyStatusBuffer.newKeys | sKeyStatusBuffer.heldKeys; 
	orrs	r2, r3	@ tmp206, tmp208
@ C_Code.c:449: 	int x = proc->anim2->xPosition; 
...
@ C_Code.c:450: 	if (x > 119) { x -= 40; } 
	subs	r5, r5, #40	@ x,
.L315:
@ C_Code.c:452: 	struct BattleUnit* active_bunit = proc->active_bunit; 
	ldr	r3, [r4, #60]	@ active_bunit, proc_8(D)->active_bunit
	mov	r9, r3	@ active_bunit, active_bunit
@ C_Code.c:453: 	struct BattleUnit* opp_bunit = proc->opp_bunit; 
	ldr	r3, [r4, #64]	@ opp_bunit, proc_8(D)->opp_bunit
	mov	r10, r3	@ opp_bunit, opp_bunit
@ C_Code.c:454: 	int hitTime = !proc->EkrEfxIsUnitHittedNowFrames; 
	movs	r3, #79	@ tmp212,
@ C_Code.c:455: 	if (hitTime) { // 1 frame 
	ldrb	r3, [r4, r3]	@ tmp213,
	cmp	r3, #0	@ tmp213,
	bne	.L317		@,
@ C_Code.c:457: 		if (proc->timer2 == 0xFF) { proc->timer2 = 0; }  
	movs	r2, #68	@ tmp214,
@ C_Code.c:457: 		if (proc->timer2 == 0xFF) { proc->timer2 = 0; }  
...
.LCB1904:
.L318:
@ C_Code.c:458: 		SaveInputFrame(proc, keys); 
	mov	r1, r8	@, keys
	movs	r0, r4	@, proc
	bl	SaveInputFrame		@
@ C_Code.c:459: 		SaveIfWeHitOnTime(proc);
	movs	r0, r4	@, proc
	bl	SaveIfWeHitOnTime		@
@ C_Code.c:460: 		if (!proc->adjustedDmg) { 
	movs	r3, #71	@ tmp219,
@ C_Code.c:460: 		if (!proc->adjustedDmg) { 
...
@ C_Code.c:461: 			if (DidWeHitOnTime(proc)) { 
...
@ C_Code.c:544: 		AdjustDamageByPercent(proc, HpProc, active_bunit, opp_bunit, round, percent);
	...
	bl	AdjustDamageByPercent		@
.L317:
@ C_Code.c:476: 	if (EkrEfxIsUnitHittedNow(proc->side) || (proc->code4frame != 0xFF) || (proc->codefframe != 0xFF) || (proc->timer2 != 0xFF)) { 
	movs	r3, #74	@ tmp242,
@ C_Code.c:476: 	if (EkrEfxIsUnitHittedNow(proc->side) || (proc->code4frame != 0xFF) || (proc->codefframe != 0xFF) || (proc->timer2 != 0xFF)) { 
	ldrb	r0, [r4, r3]	@ tmp243,
	ldr	r3, .L362+32	@ tmp244,
	bl	.L10		@
@ C_Code.c:476: 	if (EkrEfxIsUnitHittedNow(proc->side) || (proc->code4frame != 0xFF) || (proc->codefframe != 0xFF) || (proc->timer2 != 0xFF)) { 
	movs	r3, #68	@ tmp247,
...
@ C_Code.c:477: 		if (DidWeHitOnTime(proc)) { 
	ldrb	r3, [r4, r3]	@ tmp259,
	cmp	r3, #0	@ tmp259,
	bne	.L327		@,
@ C_Code.c:494: 		else if (proc->timer2 < 20) { 
	cmp	r6, #19	@ pretmp_16,
	bhi	.L329		@,
@ C_Code.c:495: 			if (ChangePaletteWhenButtonIsPressed) { 
	ldr	r2, .L362+40	@ tmp286,
@ C_Code.c:401: 	if (!DisplayPress) { return; } 
	ldr	r3, .L362+44	@ tmp285,
@ C_Code.c:495: 			if (ChangePaletteWhenButtonIsPressed) { 
	ldr	r2, [r2]	@ ChangePaletteWhenButtonIsPressed, ChangePaletteWhenButtonIsPressed
@ C_Code.c:401: 	if (!DisplayPress) { return; } 
...
@ C_Code.c:401: 	if (!DisplayPress) { return; } 
	...
	bne	.L361		@,
.L329:
@ C_Code.c:502: 		proc->roundEnd = true; 
	movs	r3, #78	@ tmp288,
	movs	r2, #1	@ tmp289,
	strb	r2, [r4, r3]	@ tmp289, proc_8(D)->roundEnd
	b	.L310		@
.L327:
@ C_Code.c:487: 			if (((y > (-16)) && (y < (161)))) { 
	movs	r3, #63	@ tmp260,
	subs	r3, r3, r6	@ tmp261, tmp260, pretmp_16
@ C_Code.c:487: 			if (((y > (-16)) && (y < (161)))) { 
	cmp	r3, #175	@ tmp261,
	bhi	.L329		@,
@ C_Code.c:488: 				if (!gBanimDoneFlag[proc->side]) { // doesn't seem to matter ? 
	movs	r2, #74	@ tmp263,
	ldrb	r2, [r4, r2]	@ tmp264,
@ C_Code.c:488: 				if (!gBanimDoneFlag[proc->side]) { // doesn't seem to matter ? 
	ldr	r3, .L362+48	@ tmp262,
	lsls	r2, r2, #2	@ tmp265, tmp264,
@ C_Code.c:488: 				if (!gBanimDoneFlag[proc->side]) { // doesn't seem to matter ? 
	ldr	r3, [r2, r3]	@ gBanimDoneFlag[_56], gBanimDoneFlag[_56]
	cmp	r3, #0	@ gBanimDoneFlag[_56],
	bne	.L329		@,
@ C_Code.c:483: 			x += xPos[Mod(clock, sizeof(xPos)+1)]; 
	movs	r1, #31	@,
	movs	r0, r6	@, pretmp_16
	ldr	r3, .L362+52	@ tmp267,
	bl	.L10		@
@ C_Code.c:483: 			x += xPos[Mod(clock, sizeof(xPos)+1)]; 
	ldr	r3, .L362+56	@ tmp268,
	movs	r2, #104	@ tmp269,
	adds	r3, r3, r0	@ tmp270, tmp268, tmp305
@ C_Code.c:486: 			y -= clock; 
	movs	r0, #48	@ tmp274,
@ C_Code.c:483: 			x += xPos[Mod(clock, sizeof(xPos)+1)]; 
	ldrb	r1, [r3, r2]	@ tmp271, xPos
@ C_Code.c:486: 			y -= clock; 
	subs	r0, r0, r6	@ y, tmp274, pretmp_16
@ C_Code.c:489: 				PutSprite(0, OAM1_X(x + 0x200), OAM0_Y(y), sSprite_Star, oam2); 
...
@ C_Code.c:489: 				PutSprite(0, OAM1_X(x + 0x200), OAM0_Y(y), sSprite_Star, oam2); 
	...
	str	r0, [sp]	@ tmp281,
	ldr	r3, .L362+60	@ tmp273,
	movs	r0, #0	@,
	ldr	r5, .L362+64	@ tmp282,
	lsrs	r1, r1, #23	@ tmp279, tmp280,
	bl	.L28		@
	b	.L329		@
.L314:
@ C_Code.c:451: 	else if (x > 40) { x -= 20; } 
	cmp	r5, #40	@ x,
	bgt	.LCB2037	@
	b	.L315	@long jump	@
.LCB2037:
...
@ C_Code.c:476: 	if (EkrEfxIsUnitHittedNow(proc->side) || (proc->code4frame != 0xFF) || (proc->codefframe != 0xFF) || (proc->timer2 != 0xFF)) { 
	...
	bne	.L325		@,
@ C_Code.c:506: 		if (proc->timer < 1) { proc->frame = 0; } // 10 frames after hitting where it's okay to have A held down 
...
@ C_Code.c:523: 		if (UNIT_FACTION(&active_bunit->unit) == FACTION_RED) { 
	...
	bne	.L322		@,
@ C_Code.c:524: 			if (BlockingEnabled) { return ReducedDamagePercent; }
	ldr	r3, .L362+68	@ tmp234,
@ C_Code.c:524: 			if (BlockingEnabled) { return ReducedDamagePercent; }
	ldr	r3, [r3]	@ BlockingEnabled, BlockingEnabled
	cmp	r3, #0	@ BlockingEnabled,
	beq	.L317		@,
@ C_Code.c:524: 			if (BlockingEnabled) { return ReducedDamagePercent; }
	ldr	r3, .L362+72	@ tmp236,
	ldr	r3, [r3]	@ _80,
@ C_Code.c:524: 			if (BlockingEnabled) { return ReducedDamagePercent; }
	b	.L357		@
.L330:
...
.L361:
	...
	b	.L329		@
.L331:
@ C_Code.c:509: 			SaveInputFrame(proc, keys); 
	mov	r1, r8	@, keys
	movs	r0, r4	@, proc
	bl	SaveInputFrame		@
	b	.L332		@
...
@ C_Code.c:266: 	if (!battleProc) { return; } // 0 after suspend until battle start 
	cmp	r5, #0	@ battleProc,
	beq	.L364		@,
@ C_Code.c:267: 	if (!proc->anim2) { return; }
	...
	beq	.L364		@,
@ C_Code.c:268: 	if (gEkrBattleEndFlag) { return; } // 0 after suspend until battle done
	ldr	r3, .L388+4	@ tmp146,
@ C_Code.c:268: 	if (gEkrBattleEndFlag) { return; } // 0 after suspend until battle done
	ldr	r3, [r3]	@ gEkrBattleEndFlag, gEkrBattleEndFlag
	cmp	r3, #0	@ gEkrBattleEndFlag,
	bne	.L364		@,
@ C_Code.c:272: 	if (proc->timer2 != 0xFF) { proc->timer2++; } 
	movs	r2, #68	@ tmp150,
@ C_Code.c:271: 	proc->timer++;
...
.L368:
@ C_Code.c:276: 	if (proc->EkrEfxIsUnitHittedNowFrames != 0xFF) { 
	movs	r6, #79	@ tmp154,
	ldrb	r3, [r4, r6]	@ _8,
@ C_Code.c:274: 	struct SkillSysBattleHit* currentRound = proc->currentRound; 
	ldr	r7, [r4, #56]	@ currentRound, proc_29(D)->currentRound
@ C_Code.c:276: 	if (proc->EkrEfxIsUnitHittedNowFrames != 0xFF) { 
	cmp	r3, #255	@ _8,
	beq	.L369		@,
@ C_Code.c:277: 		proc->EkrEfxIsUnitHittedNowFrames++; 
	adds	r3, r3, #1	@ tmp155,
	strb	r3, [r4, r6]	@ tmp155, proc_29(D)->EkrEfxIsUnitHittedNowFrames
.L370:
@ C_Code.c:282: 	struct NewProcEfxHPBar* HpProc = Proc_Find(gProcScr_efxHPBarResire); 
	ldr	r3, .L388+8	@ tmp166,
	ldr	r6, .L388+12	@ tmp168,
	ldr	r0, [r3]	@ gProcScr_efxHPBarResire, gProcScr_efxHPBarResire
	bl	.L390		@
	subs	r2, r0, #0	@ HpProc, tmp186,
@ C_Code.c:283: 	if (!HpProc) { HpProc = Proc_Find(gProcScr_efxHPBar); } 
	beq	.L371		@,
@ C_Code.c:284: 	DoStuffIfHit(proc, battleProc, HpProc, currentRound); 
...
@ C_Code.c:257: 	if (proc->EkrEfxIsUnitHittedNowFrames) { return false; } 
	...
	cmp	r3, #0	@ tmp173,
	bne	.L364		@,
@ C_Code.c:286: 		int x = DisplayDamage2(proc->anim2, 0, 0, 0, proc->roundId); 
	movs	r6, #70	@ tmp175,
@ C_Code.c:286: 		int x = DisplayDamage2(proc->anim2, 0, 0, 0, proc->roundId); 
	...
	ldr	r5, .L388+16	@ tmp177,
	str	r3, [sp]	@ tmp176,
	movs	r3, #0	@,
	bl	.L28		@
	movs	r3, r0	@ x, tmp188
@ C_Code.c:287: 		x = DisplayDamage2(proc->anim, 1, proc->anim->xPosition, x, proc->roundId);  
...
@ C_Code.c:290: } 
	...
	bx	r0
.L369:
@ C_Code.c:279: 	else if (EkrEfxIsUnitHittedNow(proc->side)) { proc->EkrEfxIsUnitHittedNowFrames = 0; } 
	movs	r3, #74	@ tmp158,
@ C_Code.c:279: 	else if (EkrEfxIsUnitHittedNow(proc->side)) { proc->EkrEfxIsUnitHittedNowFrames = 0; } 
	ldrb	r0, [r4, r3]	@ tmp159,
	ldr	r3, .L388+20	@ tmp160,
	bl	.L10		@
@ C_Code.c:279: 	else if (EkrEfxIsUnitHittedNow(proc->side)) { proc->EkrEfxIsUnitHittedNowFrames = 0; } 
	cmp	r0, #0	@ tmp185,
	beq	.L370		@,
...
GetBattleAnimPreconfType:
	...
	@ args = 0, pretend = 0, frame = 0
	@ frame_needed = 0, uses_anonymous_args = 0
@ C_Code.c:701: 	int result = gPlaySt.config.animationType; 
	movs	r2, #66	@ tmp130,
@ C_Code.c:700: int GetBattleAnimPreconfType(void) {
	push	{r4, lr}	@
@ C_Code.c:701: 	int result = gPlaySt.config.animationType; 
	ldr	r3, .L403	@ tmp164,
	ldrb	r0, [r3, r2]	@ gPlaySt, gPlaySt
@ C_Code.c:702: 	if (!CheatCodeOn()) { 
...
@ C_Code.c:702: 	if (!CheatCodeOn()) { 
	cmp	r2, #127	@ tmp139,
	bhi	.L392		@,
@ C_Code.c:703: 		if (ForceAnimsOn) { if (result == 2) { return 2; } else { return 1; } } 
	ldr	r2, .L403+4	@ tmp140,
@ C_Code.c:703: 		if (ForceAnimsOn) { if (result == 2) { return 2; } else { return 1; } } 
...
@ C_Code.c:724: }
	...
	bx	r1
.L392:
@ C_Code.c:706:     if (gPlaySt.config.animationType != PLAY_ANIMCONF_SOLO_ANIM)
	movs	r2, #66	@ tmp143,
...
@ C_Code.c:706:     if (gPlaySt.config.animationType != PLAY_ANIMCONF_SOLO_ANIM)
	...
	cmp	r3, #4	@ tmp150,
	bne	.L391		@,
@ C_Code.c:710:     if (UNIT_FACTION(&gBattleActor.unit) == FACTION_BLUE)
	movs	r1, #11	@ tmp154,
@ C_Code.c:711:         if (UNIT_FACTION(&gBattleTarget.unit) == FACTION_BLUE)
	movs	r4, #11	@ pretmp_25,
@ C_Code.c:710:     if (UNIT_FACTION(&gBattleActor.unit) == FACTION_BLUE)
	ldr	r0, .L403+8	@ tmp153,
@ C_Code.c:711:         if (UNIT_FACTION(&gBattleTarget.unit) == FACTION_BLUE)
	ldr	r2, .L403+12	@ tmp152,
@ C_Code.c:710:     if (UNIT_FACTION(&gBattleActor.unit) == FACTION_BLUE)
...
@ C_Code.c:716:         if (UNIT_FACTION(&gBattleTarget.unit) != FACTION_BLUE)
	...
	bne	.L395		@,
@ C_Code.c:723:         return GetSoloAnimPreconfType(&gBattleTarget.unit);
	movs	r0, r2	@, tmp152
.L402:
	ldr	r3, .L403+16	@ tmp162,
	bl	.L10		@
	b	.L391		@
.L404:
	.align	2
