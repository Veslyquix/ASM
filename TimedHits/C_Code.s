...
@ C_Code.c:352: 	for (int i = 0; i < 6; ++i) { 
	adds	r3, r3, #1	@ ivtmp.106,
@ C_Code.c:353: 		if (keys & RomKeysList[i]) { c++; } 
	adds	r2, r2, r4	@ c, c, tmp233
@ C_Code.c:352: 	for (int i = 0; i < 6; ++i) { 
	cmp	r1, r3	@ _76, ivtmp.106
	bne	.L64		@,
...
@ C_Code.c:360: 	int reqKeys = GetButtonsToPress(proc); 
	bl	GetButtonsToPress		@
	ldr	r5, .L151	@ tmp150,
	movs	r2, r5	@ ivtmp.140, tmp150
	adds	r2, r2, #48	@ ivtmp.140,
	mov	ip, r0	@ reqKeys, tmp181
@ C_Code.c:361: 	int count = CountKeysPressed(reqKeys); 
	movs	r1, r2	@ ivtmp.152, ivtmp.140
@ C_Code.c:351: 	int c = 0; 
	movs	r6, #0	@ c,
	adds	r5, r5, #54	@ _58,
.L127:
...
@ C_Code.c:352: 	for (int i = 0; i < 6; ++i) { 
	adds	r1, r1, #1	@ ivtmp.152,
@ C_Code.c:353: 		if (keys & RomKeysList[i]) { c++; } 
	adds	r6, r6, r3	@ c, c, tmp168
@ C_Code.c:352: 	for (int i = 0; i < 6; ++i) { 
	cmp	r5, r1	@ _58, ivtmp.152
	bne	.L127		@,
	movs	r1, r2	@ ivtmp.146, ivtmp.140
@ C_Code.c:351: 	int c = 0; 
	movs	r7, #0	@ c,
.L129:
@ C_Code.c:353: 		if (keys & RomKeysList[i]) { c++; } 
...
@ C_Code.c:352: 	for (int i = 0; i < 6; ++i) { 
	adds	r2, r2, #1	@ ivtmp.140,
@ C_Code.c:353: 		if (keys & RomKeysList[i]) { c++; } 
	adds	r1, r1, r3	@ c, c, tmp174
@ C_Code.c:352: 	for (int i = 0; i < 6; ++i) { 
	cmp	r2, r5	@ ivtmp.140, _58
	bne	.L132		@,
@ C_Code.c:362: 	if (ABS(count - CountKeysPressed(keys)) > 1) { return false; } // you pressed more than 1 extra key. Shame on you. 
...
@ C_Code.c:681: 	if (UsingSkillSys && (!ProcSkillsStackWithTimedHits) && (proc->currentRound->skillID)) { 
	...
	bne	.L287		@,
.L267:
@ C_Code.c:686: 	else { UpdateHP(proc, HpProc, opp_bunit, newHp, side, newDamage); } 
	mov	r3, r9	@ newDamage, newDamage
	...
	bl	UpdateHP		@
.L268:
@ C_Code.c:690: 	CheckForDeath(proc, HpProc, active_bunit, opp_bunit, round, newHp, newDamage); 
	mov	r3, r9	@ newDamage, newDamage
...
GetBattleAnimPreconfType:
	...
	@ args = 0, pretend = 0, frame = 0
	@ frame_needed = 0, uses_anonymous_args = 0
@ C_Code.c:705: 	int result = gPlaySt.config.animationType; 
	movs	r2, #66	@ tmp130,
@ C_Code.c:704: int GetBattleAnimPreconfType(void) {
	push	{r4, lr}	@
@ C_Code.c:705: 	int result = gPlaySt.config.animationType; 
	ldr	r3, .L403	@ tmp164,
	ldrb	r0, [r3, r2]	@ gPlaySt, gPlaySt
@ C_Code.c:706: 	if (!CheatCodeOn()) { 
...
@ C_Code.c:706: 	if (!CheatCodeOn()) { 
	cmp	r2, #127	@ tmp139,
	bhi	.L392		@,
@ C_Code.c:707: 		if (ForceAnimsOn) { if (result == 2) { return 2; } else { return 1; } } 
...
@ C_Code.c:728: }
	...
	bx	r1
.L392:
@ C_Code.c:710:     if (gPlaySt.config.animationType != PLAY_ANIMCONF_SOLO_ANIM)
	movs	r2, #66	@ tmp143,
...
@ C_Code.c:710:     if (gPlaySt.config.animationType != PLAY_ANIMCONF_SOLO_ANIM)
	...
	cmp	r3, #4	@ tmp150,
	bne	.L391		@,
@ C_Code.c:714:     if (UNIT_FACTION(&gBattleActor.unit) == FACTION_BLUE)
	movs	r1, #11	@ tmp154,
@ C_Code.c:715:         if (UNIT_FACTION(&gBattleTarget.unit) == FACTION_BLUE)
...
@ C_Code.c:714:     if (UNIT_FACTION(&gBattleActor.unit) == FACTION_BLUE)
	...
	beq	.L402		@,
@ C_Code.c:720:         if (UNIT_FACTION(&gBattleTarget.unit) != FACTION_BLUE)
	tst	r3, r4	@ tmp155, pretmp_25
	bne	.L395		@,
@ C_Code.c:727:         return GetSoloAnimPreconfType(&gBattleTarget.unit);
	movs	r0, r2	@, tmp152
.L402:
