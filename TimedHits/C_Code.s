...
GetButtonsToPress.part.0:
	...
	mov	r7, r9	@,
	push	{r6, r7, lr}	@
@ C_Code.c:300: 		u8 KeysList[] = { A_BUTTON, B_BUTTON, DPAD_RIGHT, DPAD_LEFT, DPAD_UP, DPAD_DOWN }; 
	ldr	r3, .L21	@ tmp133,
	ldr	r2, [r3]	@ tmp136,
@ C_Code.c:296: int GetButtonsToPress(TimedHitsProc* proc) { 
	sub	sp, sp, #8	@,,
@ C_Code.c:300: 		u8 KeysList[] = { A_BUTTON, B_BUTTON, DPAD_RIGHT, DPAD_LEFT, DPAD_UP, DPAD_DOWN }; 
	str	r2, [sp]	@ tmp136,
	mov	r2, sp	@ tmp166,
	ldrh	r3, [r3, #4]	@ tmp138,
	strh	r3, [r2, #4]	@ tmp138,
@ C_Code.c:305: 		int numberOfRandomButtons = NumberOfRandomButtons; 
	ldr	r3, .L21+4	@ tmp140,
...
@ C_Code.c:306: 		if (!numberOfRandomButtons) { numberOfRandomButtons = TimedHitsDifficultyRam->difficulty; } 
...
@ C_Code.c:307: 		if (!numberOfRandomButtons) { numberOfRandomButtons = 1; }
	cmp	r3, #0	@ tmp147,
	beq	.L18		@,
.L3:
	ldr	r3, .L21+12	@ tmp161,
@ C_Code.c:303: 		int oppDir = 0; 
	movs	r6, #0	@ oppDir,
@ C_Code.c:308: 		for (int i = 0; i < numberOfRandomButtons; ++i) { 
	movs	r5, #0	@ i,
@ C_Code.c:298: 	int keys = proc->buttonsToPress;
	movs	r7, #0	@ <retval>,
@ C_Code.c:304: 		int size = 5; // -1 since we count from 0  
	movs	r4, #5	@ size,
...
@ C_Code.c:308: 		for (int i = 0; i < numberOfRandomButtons; ++i) { 
	adds	r5, r5, #1	@ i,
@ C_Code.c:328: 			keys |= button; 
	orrs	r7, r0	@ <retval>, button
@ C_Code.c:308: 		for (int i = 0; i < numberOfRandomButtons; ++i) { 
	cmp	r5, r8	@ i, numberOfRandomButtons
	bge	.L19		@,
.L9:
@ C_Code.c:309: 			num = NextRN_N(size); 
	movs	r0, r4	@, size
	bl	.L23		@
@ C_Code.c:310: 			button = KeysList[num];
	mov	r3, sp	@ tmp173,
	ldrb	r0, [r3, r0]	@ button, KeysList
@ C_Code.c:313: 			if (button & 0xF0) { // some dpad 
	cmp	r0, #15	@ button,
	bls	.L5		@,
@ C_Code.c:314: 				if (button == DPAD_RIGHT) { oppDir = DPAD_LEFT; } 
	cmp	r0, #16	@ button,
	beq	.L11		@,
@ C_Code.c:315: 				if (button == DPAD_LEFT) { oppDir = DPAD_RIGHT; } 
	cmp	r0, #32	@ button,
	beq	.L12		@,
@ C_Code.c:316: 				if (button == DPAD_UP) { oppDir = DPAD_DOWN; } 
	cmp	r0, #64	@ button,
	bne	.L20		@,
@ C_Code.c:316: 				if (button == DPAD_UP) { oppDir = DPAD_DOWN; } 
	movs	r6, #128	@ oppDir,
.L6:
@ C_Code.c:318: 				for (int c = 0; c <= size; ++c) { 
	cmp	r4, #0	@ size,
	blt	.L5		@,
	mov	r2, sp	@ ivtmp.61,
@ C_Code.c:318: 				for (int c = 0; c <= size; ++c) { 
...
@ C_Code.c:308: 		for (int i = 0; i < numberOfRandomButtons; ++i) { 
	...
	blt	.L9		@,
.L19:
@ C_Code.c:330: 		proc->buttonsToPress = keys; 
...
@ C_Code.c:315: 				if (button == DPAD_LEFT) { oppDir = DPAD_RIGHT; } 
	...
	b	.L6		@
.L20:
@ C_Code.c:317: 				if (button == DPAD_DOWN) { oppDir = DPAD_UP; } 
	cmp	r0, #128	@ button,
	bne	.L6		@,
@ C_Code.c:317: 				if (button == DPAD_DOWN) { oppDir = DPAD_UP; } 
	movs	r6, #64	@ oppDir,
...
@ C_Code.c:297: 	if (AlwaysA) { return A_BUTTON; } 
	ldr	r3, .L78	@ tmp162,
@ C_Code.c:297: 	if (AlwaysA) { return A_BUTTON; } 
	ldr	r3, [r3]	@ AlwaysA, AlwaysA
@ C_Code.c:389: void DrawButtonsToPress(TimedHitsProc* proc, int x, int y, int palID) { 
	sub	sp, sp, #36	@,,
@ C_Code.c:389: void DrawButtonsToPress(TimedHitsProc* proc, int x, int y, int palID) { 
...
@ C_Code.c:298: 	int keys = proc->buttonsToPress;
	...
	ldrh	r4, [r0, r3]	@ _78,
@ C_Code.c:299: 	if (!keys) { 
	cmp	r4, #0	@ _78,
	beq	.L26		@,
.L68:
@ C_Code.c:406: 	if (keys & A_BUTTON) { 
	movs	r3, #1	@ tmp195,
	ands	r3, r4	@ tmp195, _26
	mov	fp, r3	@ _87, tmp195
@ C_Code.c:409: 	if (keys & B_BUTTON) { 
	movs	r3, #2	@ tmp196,
	ands	r3, r4	@ tmp196, _26
	str	r3, [sp, #12]	@ tmp196, %sfp
@ C_Code.c:412: 	if (keys & DPAD_LEFT) { 
	movs	r3, #32	@ tmp197,
	ands	r3, r4	@ tmp197, _26
	str	r3, [sp, #16]	@ tmp197, %sfp
@ C_Code.c:415: 	if (keys & DPAD_RIGHT) { 
	movs	r3, #16	@ tmp198,
	ands	r3, r4	@ tmp198, _26
	str	r3, [sp, #20]	@ tmp198, %sfp
@ C_Code.c:418: 	if (keys & DPAD_UP) { 
	movs	r3, #64	@ tmp199,
	ands	r3, r4	@ tmp199, _26
	str	r3, [sp, #24]	@ tmp199, %sfp
@ C_Code.c:421: 	if (keys & DPAD_DOWN) { 
	movs	r3, #128	@ tmp200,
	ands	r3, r4	@ tmp200, _26
	str	r3, [sp, #28]	@ tmp200, %sfp
.L25:
@ C_Code.c:393: 	if (ChangePaletteWhenButtonIsPressed && proc->frame) { palID = 14; } 
	ldr	r3, .L78+4	@ tmp201,
@ C_Code.c:393: 	if (ChangePaletteWhenButtonIsPressed && proc->frame) { palID = 14; } 
...
@ C_Code.c:393: 	if (ChangePaletteWhenButtonIsPressed && proc->frame) { palID = 14; } 
	...
	bne	.L69		@,
.L29:
@ C_Code.c:395: 	int oam2 = OAM2_PAL(palID) | OAM2_LAYER(0); //OAM2_CHR(0);
	lsls	r5, r5, #28	@ tmp204, palID,
	lsrs	r5, r5, #16	@ _110, tmp204,
.L28:
@ C_Code.c:396: 	PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_PressInput, oam2); 
	movs	r3, #255	@ tmp207,
	ldr	r7, [sp, #8]	@ y, %sfp
	ands	r3, r7	@ tmp207, y
...
@ C_Code.c:397: 	x += 32; 
	mov	r1, r8	@ x, x
@ C_Code.c:398: 	PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_PressInput2, oam2); 
...
@ C_Code.c:399: 	y += 16; x -= 36; 
	ldr	r7, [sp, #8]	@ y, %sfp
	movs	r1, #5	@ ivtmp_5,
	movs	r3, #1	@ pretmp_112,
@ C_Code.c:337: 	int c = 0; 
	movs	r2, #0	@ c,
@ C_Code.c:399: 	y += 16; x -= 36; 
	adds	r7, r7, #16	@ y,
	b	.L32		@
.L70:
@ C_Code.c:339: 		if (keys & RomKeysList[c]) { c++; } 
...
@ C_Code.c:338: 	for (int i = 0; i < 5; ++i) { 
...
@ C_Code.c:338: 	for (int i = 0; i < 5; ++i) { 
	cmp	r1, #0	@ ivtmp_5,
	bne	.L70		@,
@ C_Code.c:402: 	if (count == 1) { x += 24; } // centering 
	cmp	r2, #1	@ c,
	beq	.L71		@,
@ C_Code.c:403: 	if (count == 2) { x += 16; } 
	cmp	r2, #2	@ c,
	bne	.L35		@,
@ C_Code.c:403: 	if (count == 2) { x += 16; } 
	mov	r4, r8	@ x, x
...
@ C_Code.c:406: 	if (keys & A_BUTTON) { 
	...
	beq	.L37		@,
.L72:
@ C_Code.c:407: 		PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_A_Button, oam2); x += 18; 
...
@ C_Code.c:409: 	if (keys & B_BUTTON) { 
	...
	beq	.L38		@,
.L73:
@ C_Code.c:410: 		PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_B_Button, oam2); x += 18; 
...
@ C_Code.c:412: 	if (keys & DPAD_LEFT) { 
	...
	beq	.L39		@,
.L74:
@ C_Code.c:413: 		PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_Left_Button, oam2); x += 18; 
...
@ C_Code.c:415: 	if (keys & DPAD_RIGHT) { 
	...
	beq	.L40		@,
.L75:
@ C_Code.c:416: 		PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_Right_Button, oam2); x += 18; 
...
@ C_Code.c:418: 	if (keys & DPAD_UP) { 
	...
	beq	.L41		@,
.L76:
@ C_Code.c:419: 		PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_Up_Button, oam2); x += 18; 
...
@ C_Code.c:421: 	if (keys & DPAD_DOWN) { 
	...
	beq	.L24		@,
.L77:
@ C_Code.c:422: 		PutSprite(2, OAM1_X(x + 0x200), OAM0_Y(y + 0x100), sSprite_Down_Button, oam2); x += 18; 
...
@ C_Code.c:135: 	proc->anim = anim; 
	str	r5, [r0, #44]	@ anim, proc_20->anim
@ C_Code.c:136: 	proc->anim2 = GetAnimAnotherSide(anim); 
	ldr	r3, .L100+12	@ tmp150,
	movs	r0, r5	@, anim
	bl	.L85		@
@ C_Code.c:136: 	proc->anim2 = GetAnimAnotherSide(anim); 
	str	r0, [r4, #48]	@ _1, proc_20->anim2
@ C_Code.c:138: 	proc->roundId = anim->nextRoundId > proc->anim2->nextRoundId ? anim->nextRoundId-1 : proc->anim2->nextRoundId-1; 
	ldrh	r2, [r5, #14]	@ _2,
@ C_Code.c:138: 	proc->roundId = anim->nextRoundId > proc->anim2->nextRoundId ? anim->nextRoundId-1 : proc->anim2->nextRoundId-1; 
	ldrh	r3, [r0, #14]	@ _3,
@ C_Code.c:138: 	proc->roundId = anim->nextRoundId > proc->anim2->nextRoundId ? anim->nextRoundId-1 : proc->anim2->nextRoundId-1; 
...
@ C_Code.c:138: 	proc->roundId = anim->nextRoundId > proc->anim2->nextRoundId ? anim->nextRoundId-1 : proc->anim2->nextRoundId-1; 
	subs	r2, r2, #1	@ tmp152,
	lsls	r2, r2, #24	@ tmp153, tmp152,
	lsrs	r0, r2, #24	@ iftmp.1_15, tmp153,
.L96:
@ C_Code.c:138: 	proc->roundId = anim->nextRoundId > proc->anim2->nextRoundId ? anim->nextRoundId-1 : proc->anim2->nextRoundId-1; 
	movs	r3, #73	@ tmp157,
	strb	r0, [r4, r3]	@ iftmp.1_15, proc_20->roundId
@ C_Code.c:139: 	proc->currentRound = GetCurrentRound(proc->roundId); 
	ldr	r3, .L100+16	@ tmp159,
	bl	.L85		@
@ C_Code.c:140: 	proc->side = GetAnimPosition(anim) ^ 1; 
	ldr	r3, .L100+20	@ tmp160,
@ C_Code.c:139: 	proc->currentRound = GetCurrentRound(proc->roundId); 
...
@ C_Code.c:141: 	proc->active_bunit = gpEkrBattleUnitLeft; 
	ldr	r3, .L100+24	@ tmp167,
@ C_Code.c:142: 	proc->opp_bunit = gpEkrBattleUnitRight; 
	ldr	r2, .L100+28	@ tmp168,
@ C_Code.c:141: 	proc->active_bunit = gpEkrBattleUnitLeft; 
...
@ C_Code.c:143: 	if (!proc->side) { 
	cmp	r0, #1	@ _9,
	bne	.L97		@,
@ C_Code.c:144: 		proc->active_bunit = gpEkrBattleUnitRight; 
	str	r2, [r4, #64]	@ gpEkrBattleUnitRight.3_13, proc_20->active_bunit
@ C_Code.c:145: 		proc->opp_bunit = gpEkrBattleUnitLeft;
	str	r3, [r4, #68]	@ gpEkrBattleUnitLeft.2_12, proc_20->opp_bunit
.L97:
@ C_Code.c:147: 	if (!proc->loadedImg) {
	movs	r6, #76	@ tmp169,
@ C_Code.c:147: 	if (!proc->loadedImg) {
...
@ C_Code.c:138: 	proc->roundId = anim->nextRoundId > proc->anim2->nextRoundId ? anim->nextRoundId-1 : proc->anim2->nextRoundId-1; 
	...
	lsrs	r0, r3, #24	@ iftmp.1_15, tmp156,
	b	.L96		@
.L99:
@ C_Code.c:148: 		proc->timer2 = 0xFF; 
	adds	r3, r3, #255	@ tmp171,
@ C_Code.c:149: 		Copy2dChr(&Press_Image, (void*)0x06012980, 6, 2);
	ldr	r5, .L100+32	@ tmp174,
	movs	r2, #6	@,
@ C_Code.c:148: 		proc->timer2 = 0xFF; 
	str	r3, [r4, #56]	@ tmp171, proc_20->timer2
@ C_Code.c:149: 		Copy2dChr(&Press_Image, (void*)0x06012980, 6, 2);
	ldr	r0, .L100+36	@ tmp173,
	ldr	r1, .L100+40	@,
	subs	r3, r3, #253	@,
	bl	.L102		@
@ C_Code.c:150: 		Copy2dChr(&BattleStar, (void*)0x06012a40, 2, 2); // 0x108 
	movs	r3, #2	@,
	movs	r2, #2	@,
	ldr	r0, .L100+44	@ tmp176,
	ldr	r1, .L100+48	@,
	bl	.L102		@
@ C_Code.c:151: 		Copy2dChr(&A_Button, (void*)0x06012800, 2, 2); // 0x140
	movs	r3, #2	@,
	movs	r2, #2	@,
	ldr	r0, .L100+52	@ tmp179,
	ldr	r1, .L100+56	@,
	bl	.L102		@
@ C_Code.c:152: 		Copy2dChr(&B_Button, (void*)0x06012840, 2, 2); // 0x142 
	movs	r3, #2	@,
	movs	r2, #2	@,
	ldr	r0, .L100+60	@ tmp182,
	ldr	r1, .L100+64	@,
	bl	.L102		@
@ C_Code.c:153: 		Copy2dChr(&Left_Button, (void*)0x06012880, 2, 2); // 0x144
	movs	r3, #2	@,
	movs	r2, #2	@,
	ldr	r0, .L100+68	@ tmp185,
	ldr	r1, .L100+72	@,
	bl	.L102		@
@ C_Code.c:154: 		Copy2dChr(&Right_Button, (void*)0x060128C0, 2, 2); // 0x146
	movs	r3, #2	@,
	movs	r2, #2	@,
	ldr	r0, .L100+76	@ tmp188,
	ldr	r1, .L100+80	@,
	bl	.L102		@
@ C_Code.c:155: 		Copy2dChr(&Up_Button, (void*)0x06012900, 2, 2); // 0x148
	movs	r3, #2	@,
	movs	r2, #2	@,
	ldr	r0, .L100+84	@ tmp191,
	ldr	r1, .L100+88	@,
	bl	.L102		@
@ C_Code.c:156: 		Copy2dChr(&Down_Button, (void*)0x06012940, 2, 2); // 0x14a
	movs	r3, #2	@,
	movs	r2, #2	@,
	ldr	r0, .L100+92	@ tmp194,
	ldr	r1, .L100+96	@,
	bl	.L102		@
@ C_Code.c:157: 		proc->loadedImg = true;
	movs	r3, #1	@ tmp197,
...
BreakOnce:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	@ link register save eliminated.
@ C_Code.c:241: 	if (proc->broke) { return; } 
	movs	r3, #75	@ tmp116,
@ C_Code.c:241: 	if (proc->broke) { return; } 
	ldrb	r2, [r0, r3]	@ tmp117,
	cmp	r2, #0	@ tmp117,
	bne	.L103		@,
@ C_Code.c:242: 	proc->broke = true; 
	adds	r2, r2, #1	@ tmp119,
	strb	r2, [r0, r3]	@ tmp119, proc_4(D)->broke
@ C_Code.c:243: 	asm("mov r11, r11");
	.syntax divided
@ 243 "C_Code.c" 1
	mov r11, r11
@ 0 "" 2
...
HitNow:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	@ link register save eliminated.
@ C_Code.c:248: 	if (!HpProc) { return false; } // 
	cmp	r1, #0	@ tmp126,
	beq	.L107		@,
@ C_Code.c:250: 	if (proc->EkrEfxIsUnitHittedNowFrames) { return false; } 
	movs	r3, #82	@ tmp119,
@ C_Code.c:250: 	if (proc->EkrEfxIsUnitHittedNowFrames) { return false; } 
...
CountKeysPressed:
	...
	push	{r4, r5, lr}	@
@ C_Code.c:336: int CountKeysPressed(u32 keys) { 
	movs	r2, #5	@ ivtmp_1,
	movs	r3, #1	@ pretmp_5,
@ C_Code.c:337: 	int c = 0; 
	movs	r0, #0	@ <retval>,
...
@ C_Code.c:297: 	if (AlwaysA) { return A_BUTTON; } 
	ldr	r3, [r3]	@ AlwaysA, AlwaysA
@ C_Code.c:345: int PressedSpecificKeys(TimedHitsProc* proc, u32 keys) { 
...
@ C_Code.c:299: 	if (!keys) { 
	...
	beq	.L124		@,
.L151:
@ C_Code.c:347: 	int count = CountKeysPressed(reqKeys); 
...
@ C_Code.c:338: 	for (int i = 0; i < 5; ++i) { 
	...
	movs	r0, #5	@ ivtmp_62,
	movs	r3, #1	@ pretmp_78,
@ C_Code.c:348: 	if (ABS(count - CountKeysPressed(keys)) > 1) { return false; } // you pressed more than 1 extra key. Shame on you. 
	cmp	r2, r1	@ c, c
	blt	.L140		@,
@ C_Code.c:337: 	int c = 0; 
	movs	r1, #0	@ c,
@ C_Code.c:339: 		if (keys & RomKeysList[c]) { c++; } 
	ldr	r6, .L156+4	@ tmp175,
	b	.L133		@
.L154:
...
@ C_Code.c:348: 	if (ABS(count - CountKeysPressed(keys)) > 1) { return false; } // you pressed more than 1 extra key. Shame on you. 
	...
	bgt	.L142		@,
.L135:
@ C_Code.c:349: 	return (keys & reqKeys); 
...
SaveInputFrame:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	push	{r4, r5, r6, lr}	@
@ C_Code.c:352: 	struct Anim* anim = proc->anim; 
	ldr	r5, [r0, #44]	@ anim, proc_17(D)->anim
@ C_Code.c:353: 	u32 instruction = *anim->pScrCurrent++; 
	ldr	r6, [r5, #32]	@ _1, anim_18->pScrCurrent
@ C_Code.c:353: 	u32 instruction = *anim->pScrCurrent++; 
	adds	r3, r6, #4	@ tmp130, _1,
	str	r3, [r5, #32]	@ tmp130, anim_18->pScrCurrent
@ C_Code.c:354: 	if (ANINS_GET_TYPE(instruction) == ANIM_INS_TYPE_COMMAND) {
	movs	r3, #252	@ tmp132,
@ C_Code.c:351: void SaveInputFrame(TimedHitsProc* proc, u32 keys) { 
...
@ C_Code.c:354: 	if (ANINS_GET_TYPE(instruction) == ANIM_INS_TYPE_COMMAND) {
	...
	beq	.L165		@,
.L159:
@ C_Code.c:363: 	if (PressedSpecificKeys(proc, keys)) { 
	movs	r0, r4	@, proc
@ C_Code.c:362: 	instruction = *anim->pScrCurrent--; 
...
@ C_Code.c:369: }  
	...
	bx	r0
.L165:
@ C_Code.c:355: 		if (ANINS_COMMAND_GET_ID(instruction) == 4) {
	movs	r3, #255	@ tmp134,
	ands	r3, r2	@ _4, instruction
@ C_Code.c:355: 		if (ANINS_COMMAND_GET_ID(instruction) == 4) {
	cmp	r3, #4	@ _4,
	beq	.L167		@,
@ C_Code.c:358: 		if (ANINS_COMMAND_GET_ID(instruction) == 0xF) {
	cmp	r3, #15	@ _4,
	bne	.L159		@,
@ C_Code.c:359: 			proc->codefframe = proc->timer; proc->timer2 = 0; 
...
@ C_Code.c:359: 			proc->codefframe = proc->timer; proc->timer2 = 0; 
	...
	b	.L159		@
.L166:
@ C_Code.c:366: 			PlaySFX(0x13e, 0x100, 120, 1); //PlaySFX(int songid, int volume, int locate, int type)
	movs	r1, #128	@,
	movs	r0, #159	@,
@ C_Code.c:365: 			proc->frame = proc->timer; // locate is side for stereo? 
	ldr	r2, [r4, #52]	@ proc_17(D)->timer, proc_17(D)->timer
@ C_Code.c:366: 			PlaySFX(0x13e, 0x100, 120, 1); //PlaySFX(int songid, int volume, int locate, int type)
	lsls	r1, r1, #1	@,,
@ C_Code.c:365: 			proc->frame = proc->timer; // locate is side for stereo? 
...
@ C_Code.c:369: }  
	b	.L158		@
.L167:
@ C_Code.c:356: 			proc->code4frame = proc->timer; proc->timer2 = 0; 
	ldr	r2, [r4, #52]	@ proc_17(D)->timer, proc_17(D)->timer
...
@ C_Code.c:371: 	if (proc->frame) { 
	movs	r3, #78	@ tmp128,
	ldrb	r3, [r0, r3]	@ _1,
@ C_Code.c:370: void SaveIfWeHitOnTime(TimedHitsProc* proc) {
	push	{r4, lr}	@
@ C_Code.c:371: 	if (proc->frame) { 
	cmp	r3, #0	@ _1,
	beq	.L171		@,
@ C_Code.c:372: 		if (proc->codefframe != 0xFF) { if (ABS(proc->codefframe - proc->frame) < (LenienceFrames)) { proc->hitOnTime = true; } }
	movs	r2, #80	@ tmp129,
@ C_Code.c:372: 		if (proc->codefframe != 0xFF) { if (ABS(proc->codefframe - proc->frame) < (LenienceFrames)) { proc->hitOnTime = true; } }
	ldr	r1, .L182	@ tmp130,
@ C_Code.c:372: 		if (proc->codefframe != 0xFF) { if (ABS(proc->codefframe - proc->frame) < (LenienceFrames)) { proc->hitOnTime = true; } }
...
.L175:
@ C_Code.c:374: 		if ((proc->timer - proc->frame) < LenienceFrames) { proc->hitOnTime = true; } 
...
@ C_Code.c:377: }
	...
	bx	r0
.L174:
@ C_Code.c:373: 		else if (proc->code4frame != 0xFF) { if (ABS(proc->code4frame - proc->frame) < (LenienceFrames)) { proc->hitOnTime = true; } } 
	movs	r2, #79	@ tmp136,
	ldrb	r2, [r0, r2]	@ _8,
@ C_Code.c:373: 		else if (proc->code4frame != 0xFF) { if (ABS(proc->code4frame - proc->frame) < (LenienceFrames)) { proc->hitOnTime = true; } } 
	cmp	r2, #255	@ _8,
...
CheatCodeOn:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	@ link register save eliminated.
@ C_Code.c:380: 	return (gPlaySt.unk1F & 0x80); // 202BD0F
	ldr	r3, .L185	@ tmp118,
@ C_Code.c:381: } 
	@ sp needed	@
@ C_Code.c:380: 	return (gPlaySt.unk1F & 0x80); // 202BD0F
...
@ C_Code.c:380: 	return (gPlaySt.unk1F & 0x80); // 202BD0F
	ldr	r3, .L191	@ tmp120,
@ C_Code.c:384: 	if (CheatCodeOn()) { return true; } 
	ldrb	r3, [r3, #31]	@ tmp123,
	cmp	r3, #127	@ tmp123,
	bhi	.L190		@,
@ C_Code.c:385: 	if (AlwaysWork) { return true; } 
	ldr	r3, .L191+4	@ tmp124,
@ C_Code.c:385: 	if (AlwaysWork) { return true; } 
	ldr	r3, [r3]	@ AlwaysWork, AlwaysWork
	cmp	r3, #0	@ AlwaysWork,
	bne	.L190		@,
@ C_Code.c:386: 	return proc->hitOnTime;
...
DrawButtonsToPress:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	push	{r4, lr}	@
@ C_Code.c:390: 	if (!DisplayPress) { return; } 
	ldr	r4, .L198	@ tmp119,
@ C_Code.c:390: 	if (!DisplayPress) { return; } 
...
GetDefaultDamagePercent:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	@ link register save eliminated.
@ C_Code.c:508: 		if (UNIT_FACTION(&active_bunit->unit) == FACTION_RED) { 
	movs	r1, #11	@ tmp122,
	movs	r3, #192	@ tmp123,
	ldrsb	r1, [r0, r1]	@ tmp122,
	ands	r3, r1	@ _14, tmp122
@ C_Code.c:507: 	if (success) { 
	cmp	r2, #0	@ tmp131,
	beq	.L201		@,
@ C_Code.c:508: 		if (UNIT_FACTION(&active_bunit->unit) == FACTION_RED) { 
	cmp	r3, #128	@ _14,
	bne	.L202		@,
@ C_Code.c:509: 			if (BlockingEnabled) { return ReducedDamagePercent; }
	ldr	r3, .L206	@ tmp124,
@ C_Code.c:509: 			if (BlockingEnabled) { return ReducedDamagePercent; }
...
@ C_Code.c:514: 	if (UNIT_FACTION(&active_bunit->unit) == FACTION_RED) { 
	cmp	r3, #128	@ _14,
	beq	.L205		@,
@ C_Code.c:517: 	return FailedHitDamagePercent; 
...
@ C_Code.c:518: } 
	...
	bx	lr
.L202:
@ C_Code.c:512: 		return BonusDamagePercent; 
	ldr	r3, .L206+12	@ tmp127,
	ldr	r0, [r3]	@ <retval>,
	b	.L200		@
.L205:
@ C_Code.c:510: 			else { return 100; } 
	movs	r0, #100	@ <retval>,
	b	.L200		@
.L207:
	.align	2
...
@ C_Code.c:510: 			else { return 100; } 
	movs	r0, #100	@ <retval>,
@ C_Code.c:521: 	return GetDefaultDamagePercent(active_bunit, opp_bunit, success); 
	b	.L208		@
.L215:
	.align	2
.L214:
...
AdjustAllRounds:
	...
	@ frame_needed = 0, uses_anonymous_args = 0
	push	{r4, r5, lr}	@
@ C_Code.c:526: 	for (int i = id; i < 22; i += 2) {
	cmp	r0, #21	@ id,
	bgt	.L216		@,
	ldr	r3, .L226	@ tmp128,
	lsls	r2, r0, #1	@ tmp127, id,
@ C_Code.c:528: 		if (hp == 0xffff) { break; }
	ldr	r5, .L226+4	@ tmp129,
	adds	r2, r2, r3	@ ivtmp.186, tmp127, tmp128
	b	.L220		@
.L218:
	movs	r4, #0	@ _4,
@ C_Code.c:530: 		else if (hp >= difference) { gEfxHpLut[i] -= difference; }
...
@ C_Code.c:526: 	for (int i = id; i < 22; i += 2) {
	adds	r0, r0, #2	@ id,
@ C_Code.c:529: 		if (difference < 0) { hp += difference; if (hp > 0) { gEfxHpLut[i] = hp; } else { gEfxHpLut[i] = 0; } }
...
@ C_Code.c:556: 	int side = proc->side; 
	movs	r2, #77	@ tmp143,
@ C_Code.c:555: void CheckForDeath(TimedHitsProc* proc, struct NewProcEfxHPBar* HpProc, struct BattleUnit* active_bunit, struct BattleUnit* opp_bunit, struct SkillSysBattleHit* round, int hp) { 
	push	{lr}	@
@ C_Code.c:555: void CheckForDeath(TimedHitsProc* proc, struct NewProcEfxHPBar* HpProc, struct BattleUnit* active_bunit, struct BattleUnit* opp_bunit, struct SkillSysBattleHit* round, int hp) { 
...
@ C_Code.c:556: 	int side = proc->side; 
	mov	r8, r3	@ _1, _1
@ C_Code.c:560: 	if (hp < 0) { hp = GetEfxHp(id) - round->hpChange; } // + round->hpChange; 
	cmp	r0, #0	@ hp,
	blt	.L232		@,
.L229:
@ C_Code.c:561: 	if (hp <= 0) { // they are dead 
	cmp	r0, #0	@ hp,
	ble	.L233		@,
.L228:
@ C_Code.c:602: }
	...
	bx	r0
.L233:
@ C_Code.c:567: 		opp_bunit->unit.curHP = 0; 
	movs	r3, #0	@ tmp153,
	strb	r3, [r7, #19]	@ tmp153, opp_bunit_32(D)->unit.curHP
@ C_Code.c:568: 		if (UsingSkillSys) { HpProc->post = 0; } 
	ldr	r3, .L234	@ tmp155,
@ C_Code.c:568: 		if (UsingSkillSys) { HpProc->post = 0; } 
	ldr	r3, [r3]	@ UsingSkillSys, UsingSkillSys
	cmp	r3, #0	@ UsingSkillSys,
	bne	.L231		@,
@ C_Code.c:569: 		else { HpProc->post = 0; HpProc->postHpAtkrSS = 0; } 
	movs	r2, #82	@ tmp157,
...
@ C_Code.c:571: 		proc->code4frame = 0xff;
	subs	r3, r3, #1	@ tmp163,
	adds	r2, r2, #255	@ tmp164,
	strb	r2, [r4, r3]	@ tmp164, proc_24(D)->code4frame
@ C_Code.c:575: 		HpProc->death = true; 
	subs	r3, r3, #78	@ tmp167,
	subs	r2, r2, #214	@ tmp166,
	strb	r3, [r5, r2]	@ tmp167, HpProc_34(D)->death
@ C_Code.c:580: 		proc->anim->nextRoundId = 8; // seems to mostly work for now? see GetAnimNextRoundType
	ldr	r1, [r4, #44]	@ proc_24(D)->anim, proc_24(D)->anim
	subs	r2, r2, #33	@ tmp170,
	strh	r2, [r1, #14]	@ tmp170, _10->nextRoundId
@ C_Code.c:581: 		proc->anim2->nextRoundId = 8; 
	ldr	r1, [r4, #48]	@ proc_24(D)->anim2, proc_24(D)->anim2
	strh	r2, [r1, #14]	@ tmp170, _11->nextRoundId
@ C_Code.c:583: 		gBanimDoneFlag[0] = true; // stop counterattacks ?
	ldr	r2, .L234+4	@ tmp175,
	str	r3, [r2]	@ tmp167, gBanimDoneFlag[0]
@ C_Code.c:584: 		gBanimDoneFlag[1] = true; // [201fb04..201fb07]!! - nothing else is writing to it. good. 
	str	r3, [r2, #4]	@ tmp167, gBanimDoneFlag[1]
@ C_Code.c:586: 		round->info |= BATTLE_HIT_INFO_FINISHES | BATTLE_HIT_INFO_KILLS_TARGET | BATTLE_HIT_INFO_END; 
	movs	r2, #176	@ tmp179,
	...
	strb	r3, [r2, #2]	@ tmp184,
@ C_Code.c:591: 		side = 1 ^ side; 
	movs	r3, #1	@ tmp189,
	mov	r2, r8	@ _1, _1
	eors	r2, r3	@ _1, tmp189
	movs	r3, r2	@ side, _1
@ C_Code.c:592: 		id = (gEfxHpLutOff[side] * 2) + (side);
	ldr	r2, .L234+8	@ tmp190,
...
@ C_Code.c:593: 		hp = GetEfxHp(id); 
	ldr	r3, .L234+12	@ tmp195,
	bl	.L85		@
@ C_Code.c:595: 		active_bunit->unit.curHP = hp; 
	strb	r0, [r6, #19]	@ tmp205, active_bunit_48(D)->unit.curHP
@ C_Code.c:602: }
	b	.L228		@
.L232:
@ C_Code.c:559: 	int id = (gEfxHpLutOff[side] * 2) + (side);
	mov	r2, r8	@ _1, _1
	ldr	r3, .L234+8	@ tmp144,
	lsls	r2, r2, #1	@ tmp145, _1,
	ldrsh	r0, [r2, r3]	@ tmp146, gEfxHpLutOff
@ C_Code.c:559: 	int id = (gEfxHpLutOff[side] * 2) + (side);
	lsls	r0, r0, #1	@ tmp147, tmp146,
@ C_Code.c:560: 	if (hp < 0) { hp = GetEfxHp(id) - round->hpChange; } // + round->hpChange; 
	ldr	r3, .L234+12	@ tmp149,
@ C_Code.c:559: 	int id = (gEfxHpLutOff[side] * 2) + (side);
	add	r0, r0, r8	@ id, _1
@ C_Code.c:560: 	if (hp < 0) { hp = GetEfxHp(id) - round->hpChange; } // + round->hpChange; 
	bl	.L85		@
...
@ C_Code.c:607: 	int side = proc->side; 
	movs	r3, #77	@ tmp216,
@ C_Code.c:604: void AdjustDamageByPercent(TimedHitsProc* proc, struct NewProcEfxHPBar* HpProc, struct BattleUnit* active_bunit, struct BattleUnit* opp_bunit, struct SkillSysBattleHit* round, int percent) { 
	push	{r5, r6, r7, lr}	@
	mov	fp, r2	@ active_bunit, tmp348
@ C_Code.c:607: 	int side = proc->side; 
	ldrb	r2, [r0, r3]	@ side,
@ C_Code.c:608: 	int id = (gEfxHpLutOff[side] * 2) + (side);
	ldr	r3, .L270	@ tmp217,
@ C_Code.c:604: void AdjustDamageByPercent(TimedHitsProc* proc, struct NewProcEfxHPBar* HpProc, struct BattleUnit* active_bunit, struct BattleUnit* opp_bunit, struct SkillSysBattleHit* round, int percent) { 
...
@ C_Code.c:609: 	int hp = GetEfxHp(id); // + round->hpChange; 
	movs	r0, r3	@, id
	ldr	r3, .L270+4	@ tmp221,
	bl	.L85		@
	subs	r6, r0, #0	@ tmp222, tmp350,
@ C_Code.c:610: 	if (!hp) { return; } 
	beq	.L236		@,
@ C_Code.c:612: 	int damage = (round->hpChange * percent) / 100; 
...
@ C_Code.c:612: 	int damage = (round->hpChange * percent) / 100; 
	ldr	r3, .L270+8	@ tmp227,
	bl	.L85		@
	subs	r1, r0, #0	@ damage, tmp351,
@ C_Code.c:613: 	if (!damage) { damage = 1; } 
	bne	.L238		@,
@ C_Code.c:613: 	if (!damage) { damage = 1; } 
	adds	r1, r1, #1	@ damage,
.L238:
@ C_Code.c:615: 	if (damage > round->hpChange) { 
	cmp	r10, r1	@ _11, damage
	blt	.L265		@,
@ C_Code.c:636: 	else if (round->hpChange != hp) { 
	cmp	r6, r10	@ tmp222, _11
	bne	.LCB1511	@
	b	.L247	@long jump	@
.LCB1511:
@ C_Code.c:637: 		damage = round->hpChange - damage; 
	mov	r3, r10	@ _11, _11
	subs	r0, r3, r1	@ damage, _11, damage
@ C_Code.c:639: 		if (UsingSkillSys) { // uggggh 
	ldr	r3, .L270+12	@ tmp259,
	ldr	r2, [r3]	@ UsingSkillSys.24_50, UsingSkillSys
@ C_Code.c:648: 		opp_bunit->unit.curHP += damage; 
	mov	r3, r8	@ opp_bunit, opp_bunit
@ C_Code.c:649: 		round->hpChange -= damage; 
	mov	r5, r10	@ _11, _11
@ C_Code.c:648: 		opp_bunit->unit.curHP += damage; 
...
@ C_Code.c:639: 		if (UsingSkillSys) { // uggggh 
	...
	beq	.L248		@,
@ C_Code.c:640: 			HpProc->post += damage;
...
@ C_Code.c:650: 		if (UsingSkillSys == 2) { round->overDmg += damage; } // used by Huichelaar's banim numbers 
...
@ C_Code.c:650: 		if (UsingSkillSys == 2) { round->overDmg += damage; } // used by Huichelaar's banim numbers 
	...
	b	.L250		@
.L265:
@ C_Code.c:619: 		if (hp < 0) { damage -= ABS(hp); } 
	mov	r3, r10	@ _11, _11
@ C_Code.c:616: 		hp -= damage;
	subs	r0, r6, r1	@ hp, tmp222, damage
@ C_Code.c:617: 		damage -= round->hpChange; 
	subs	r1, r1, r3	@ damage, damage, _11
@ C_Code.c:619: 		if (hp < 0) { damage -= ABS(hp); } 
	cmp	r0, #0	@ hp,
	blt	.L266		@,
.L241:
@ C_Code.c:623: 		if (UsingSkillSys) { // uggggh 
	ldr	r3, .L270+12	@ tmp229,
	ldr	r2, [r3]	@ UsingSkillSys.22_20, UsingSkillSys
@ C_Code.c:632: 		opp_bunit->unit.curHP -= damage; 
	mov	r3, r8	@ opp_bunit, opp_bunit
@ C_Code.c:693: 	if (hp < 0) { hp = 0; } 
...
@ C_Code.c:633: 		round->hpChange += damage; 
...
@ C_Code.c:623: 		if (UsingSkillSys) { // uggggh 
	...
	bne	.L267		@,
@ C_Code.c:627: 			post = HpProc->postHpAtkrSS; // we only need the lower 16 bits anyway 
	movs	r0, #82	@ tmp247,
@ C_Code.c:627: 			post = HpProc->postHpAtkrSS; // we only need the lower 16 bits anyway 
	ldrsh	r2, [r7, r0]	@ post,
@ C_Code.c:628: 			post -= damage; 
	subs	r2, r2, r1	@ post, post, damage
@ C_Code.c:629: 			HpProc->postHpAtkrSS = post; 
	strh	r2, [r7, r0]	@ post, HpProc_21(D)->postHpAtkrSS
@ C_Code.c:630: 			HpProc->post = post>>16; 
...
.L246:
@ C_Code.c:692: 	AdjustAllRounds(id, damage, round->hpChange);
	movs	r2, r4	@, _197
	ldr	r0, [sp, #8]	@, %sfp
	bl	AdjustAllRounds		@
@ C_Code.c:694: 	CheckForDeath(proc, HpProc, active_bunit, opp_bunit, round, hp); 
...
@ C_Code.c:709: } 
	...
	bx	r0
.L267:
@ C_Code.c:624: 			HpProc->post -= damage;
	movs	r5, #80	@ tmp239,
@ C_Code.c:624: 			HpProc->post -= damage;
...
@ C_Code.c:633: 		round->hpChange += damage; 
	...
	strb	r4, [r3, #3]	@ _220, round_10(D)->hpChange
@ C_Code.c:634: 		if (UsingSkillSys == 2) { round->overDmg -= damage; } // used by Huichelaar's banim numbers 
...
@ C_Code.c:619: 		if (hp < 0) { damage -= ABS(hp); } 
	...
	b	.L241		@
.L248:
@ C_Code.c:643: 			post = HpProc->postHpAtkrSS; // we only need the lower 16 bits anyway 
	movs	r2, #82	@ tmp277,
@ C_Code.c:643: 			post = HpProc->postHpAtkrSS; // we only need the lower 16 bits anyway 
	ldrsh	r5, [r7, r2]	@ post,
	mov	ip, r5	@ post, post
@ C_Code.c:644: 			post += damage; 
	add	r0, r0, ip	@ post, post
@ C_Code.c:645: 			HpProc->postHpAtkrSS = post; 
	strh	r0, [r7, r2]	@ post, HpProc_21(D)->postHpAtkrSS
@ C_Code.c:646: 			HpProc->post = post>>16; 
	subs	r2, r2, #2	@ tmp281,
@ C_Code.c:646: 			HpProc->post = post>>16; 
...
@ C_Code.c:658: 			if (UsingSkillSys) { // uggggh 
	ldr	r3, .L270+12	@ tmp289,
	ldr	r6, [r3]	@ prephitmp_210, UsingSkillSys
@ C_Code.c:668: 			round->hpChange += damage; 
	movs	r3, #2	@ _233,
@ C_Code.c:658: 			if (UsingSkillSys) { // uggggh 
...
@ C_Code.c:669: 			if (UsingSkillSys == 2) { round->overDmg -= damage; } 
...
@ C_Code.c:512: 		return BonusDamagePercent; 
	...
	ldr	r4, [r4]	@ _19,
.L276:
@ C_Code.c:550: 	if (percent != 100) { 
	cmp	r4, #100	@ _19,
	beq	.L272		@,
@ C_Code.c:606: 	if ((proc->currentRound->attributes & BATTLE_HIT_ATTR_MISS) || (!proc->currentRound->hpChange)) { return; } 
...
DoStuffIfHit.part.0:
	...
	mov	lr, r10	@,
	push	{r6, r7, lr}	@
@ C_Code.c:433: 	u32 keys = sKeyStatusBuffer.newKeys | sKeyStatusBuffer.heldKeys; 
	ldr	r3, .L330	@ tmp164,
@ C_Code.c:433: 	u32 keys = sKeyStatusBuffer.newKeys | sKeyStatusBuffer.heldKeys; 
	ldrh	r5, [r3, #8]	@ tmp167,
	ldrh	r3, [r3, #4]	@ tmp169,
...
@ C_Code.c:437: 	int x = proc->anim2->xPosition; 
	ldr	r3, [r0, #48]	@ proc_5(D)->anim2, proc_5(D)->anim2
@ C_Code.c:431: void DoStuffIfHit(TimedHitsProc* proc, struct ProcEkrBattle* battleProc, struct NewProcEfxHPBar* HpProc, struct SkillSysBattleHit* round) { 
	movs	r7, r2	@ round, tmp259
@ C_Code.c:437: 	int x = proc->anim2->xPosition; 
	movs	r2, #2	@ tmp268,
	ldrsh	r3, [r3, r2]	@ x, proc_5(D)->anim2, tmp268
	mov	r8, r3	@ x, x
@ C_Code.c:438: 	struct BattleUnit* active_bunit = proc->active_bunit; 
	ldr	r3, [r0, #64]	@ active_bunit, proc_5(D)->active_bunit
	mov	r9, r3	@ active_bunit, active_bunit
@ C_Code.c:439: 	struct BattleUnit* opp_bunit = proc->opp_bunit; 
	ldr	r3, [r0, #68]	@ opp_bunit, proc_5(D)->opp_bunit
	mov	r10, r3	@ opp_bunit, opp_bunit
@ C_Code.c:440: 	int hitTime = !proc->EkrEfxIsUnitHittedNowFrames; 
	movs	r3, #82	@ tmp173,
@ C_Code.c:441: 	if (hitTime) { // 1 frame 
...
@ C_Code.c:441: 	if (hitTime) { // 1 frame 
	cmp	r3, #0	@ tmp174,
	bne	.L296		@,
@ C_Code.c:443: 		if (proc->timer2 == 0xFF) { proc->timer2 = 0; }  
...
.LCB1908:
.L297:
@ C_Code.c:444: 		SaveInputFrame(proc, keys); 
	movs	r1, r5	@, keys
	movs	r0, r4	@, proc
	bl	SaveInputFrame		@
@ C_Code.c:445: 		SaveIfWeHitOnTime(proc);
	movs	r0, r4	@, proc
	bl	SaveIfWeHitOnTime		@
@ C_Code.c:446: 		if (!proc->adjustedDmg) { 
	movs	r3, #74	@ tmp177,
@ C_Code.c:446: 		if (!proc->adjustedDmg) { 
	...
	bne	.L296		@,
@ C_Code.c:380: 	return (gPlaySt.unk1F & 0x80); // 202BD0F
	ldr	r2, .L330+4	@ tmp179,
@ C_Code.c:384: 	if (CheatCodeOn()) { return true; } 
	ldrb	r2, [r2, #31]	@ tmp182,
	cmp	r2, #127	@ tmp182,
	bhi	.L299		@,
@ C_Code.c:385: 	if (AlwaysWork) { return true; } 
	ldr	r2, .L330+8	@ tmp183,
@ C_Code.c:385: 	if (AlwaysWork) { return true; } 
...
@ C_Code.c:447: 			if (DidWeHitOnTime(proc)) { 
...
@ 0 "" 2
	...
	.syntax unified
.L301:
@ C_Code.c:449: 				proc->adjustedDmg = true; 
	movs	r3, #1	@ tmp193,
	movs	r2, #74	@ tmp192,
@ C_Code.c:450: 				AdjustDamageWithGetter(proc, HpProc, active_bunit, opp_bunit, round, true); 
	movs	r1, r6	@, HpProc
@ C_Code.c:449: 				proc->adjustedDmg = true; 
...
.L302:
@ C_Code.c:459: 			CheckForDeath(proc, HpProc, active_bunit, opp_bunit, round, (-1)); 
	movs	r3, #1	@ tmp210,
	...
	bl	CheckForDeath		@
.L296:
@ C_Code.c:464: 	if (EkrEfxIsUnitHittedNow(proc->side) || (proc->code4frame != 0xFF) || (proc->codefframe != 0xFF)) { 
	movs	r3, #77	@ tmp211,
@ C_Code.c:464: 	if (EkrEfxIsUnitHittedNow(proc->side) || (proc->code4frame != 0xFF) || (proc->codefframe != 0xFF)) { 
	ldrb	r0, [r4, r3]	@ tmp212,
	ldr	r3, .L330+16	@ tmp213,
	bl	.L85		@
@ C_Code.c:464: 	if (EkrEfxIsUnitHittedNow(proc->side) || (proc->code4frame != 0xFF) || (proc->codefframe != 0xFF)) { 
	cmp	r0, #0	@ tmp214,
	bne	.L303		@,
...
.L303:
@ C_Code.c:380: 	return (gPlaySt.unk1F & 0x80); // 202BD0F
	ldr	r3, .L330+4	@ tmp220,
@ C_Code.c:384: 	if (CheatCodeOn()) { return true; } 
	ldrb	r3, [r3, #31]	@ tmp223,
@ C_Code.c:484: 		else if (proc->timer2 < 20) { 
	ldr	r5, [r4, #56]	@ pretmp_99, proc_5(D)->timer2
@ C_Code.c:384: 	if (CheatCodeOn()) { return true; } 
	cmp	r3, #127	@ tmp223,
	bhi	.L305		@,
@ C_Code.c:385: 	if (AlwaysWork) { return true; } 
	ldr	r3, .L330+8	@ tmp224,
@ C_Code.c:385: 	if (AlwaysWork) { return true; } 
	ldr	r3, [r3]	@ AlwaysWork, AlwaysWork
	cmp	r3, #0	@ AlwaysWork,
	bne	.L305		@,
...
@ C_Code.c:479: 			if (((y > (-16)) && (y < (161)))) { 
	...
	bls	.L329		@,
.L307:
@ C_Code.c:487: 		proc->roundEnd = true; 
	movs	r3, #81	@ tmp245,
	movs	r2, #1	@ tmp246,
...
@ C_Code.c:464: 	if (EkrEfxIsUnitHittedNow(proc->side) || (proc->code4frame != 0xFF) || (proc->codefframe != 0xFF)) { 
	...
	cmp	r3, #255	@ tmp219,
	bne	.L303		@,
@ C_Code.c:491: 		if (proc->timer < 1) { proc->frame = 0; } // 10 frames after hitting where it's okay to have A held down 
...
@ C_Code.c:390: 	if (!DisplayPress) { return; } 
	...
	b	.L294		@
.L329:
@ C_Code.c:474: 			x += Mod(clock, 8) >> 1; 
	movs	r1, #8	@,
	movs	r0, r5	@, pretmp_99
	ldr	r3, .L330+24	@ tmp230,
	bl	.L85		@
@ C_Code.c:476: 			y -= clock; 
	movs	r1, #48	@ tmp233,
...
@ C_Code.c:480: 				PutSprite(0, OAM1_X(x + 0x200), OAM0_Y(y), sSprite_Star, oam2); 
	...
	str	r0, [sp]	@ tmp241,
	ldr	r3, .L330+28	@ tmp232,
	movs	r0, #0	@,
	ldr	r5, .L330+32	@ tmp242,
	lsrs	r1, r1, #23	@ tmp239, tmp240,
	bl	.L102		@
	b	.L307		@
.L309:
@ C_Code.c:494: 			SaveInputFrame(proc, keys); 
	movs	r1, r5	@, keys
	movs	r0, r4	@, proc
	bl	SaveInputFrame		@
	b	.L310		@
...
@ C_Code.c:93: 	return !CheckFlag(DisabledFlag); 
	ldr	r0, [r3]	@ DisabledFlag, DisabledFlag
	ldr	r3, .L334+4	@ tmp122,
@ C_Code.c:431: void DoStuffIfHit(TimedHitsProc* proc, struct ProcEkrBattle* battleProc, struct NewProcEfxHPBar* HpProc, struct SkillSysBattleHit* round) { 
	movs	r5, r2	@ HpProc, tmp126
@ C_Code.c:93: 	return !CheckFlag(DisabledFlag); 
	bl	.L85		@
@ C_Code.c:432: 	if (!AreTimedHitsEnabled()) { return; } 
	cmp	r0, #0	@ tmp128,
	bne	.L332		@,
	movs	r2, r6	@, round
	movs	r1, r5	@, HpProc
...
.L340:
@ C_Code.c:267: 	if (proc->EkrEfxIsUnitHittedNowFrames != 0xFF) { 
	movs	r5, #82	@ tmp149,
	ldrb	r3, [r4, r5]	@ _7,
@ C_Code.c:265: 	struct SkillSysBattleHit* currentRound = proc->currentRound; 
	ldr	r6, [r4, #60]	@ currentRound, proc_25(D)->currentRound
@ C_Code.c:267: 	if (proc->EkrEfxIsUnitHittedNowFrames != 0xFF) { 
	cmp	r3, #255	@ _7,
	beq	.L341		@,
@ C_Code.c:268: 		proc->EkrEfxIsUnitHittedNowFrames++; 
	adds	r3, r3, #1	@ tmp150,
	strb	r3, [r4, r5]	@ tmp150, proc_25(D)->EkrEfxIsUnitHittedNowFrames
.L342:
@ C_Code.c:273: 	struct NewProcEfxHPBar* HpProc = Proc_Find(gProcScr_efxHPBar); 
	ldr	r3, .L359+4	@ tmp161,
	ldr	r0, [r3]	@ gProcScr_efxHPBar, gProcScr_efxHPBar
	ldr	r3, .L359+8	@ tmp163,
	bl	.L85		@
@ C_Code.c:93: 	return !CheckFlag(DisabledFlag); 
	ldr	r3, .L359+12	@ tmp164,
@ C_Code.c:273: 	struct NewProcEfxHPBar* HpProc = Proc_Find(gProcScr_efxHPBar); 
	movs	r5, r0	@ HpProc, tmp183
@ C_Code.c:93: 	return !CheckFlag(DisabledFlag); 
	ldr	r0, [r3]	@ DisabledFlag, DisabledFlag
	ldr	r3, .L359+16	@ tmp166,
	bl	.L85		@
@ C_Code.c:432: 	if (!AreTimedHitsEnabled()) { return; } 
	cmp	r0, #0	@ tmp184,
	bne	.L343		@,
...
@ C_Code.c:250: 	if (proc->EkrEfxIsUnitHittedNowFrames) { return false; } 
	...
	cmp	r3, #0	@ tmp170,
	bne	.L336		@,
@ C_Code.c:276: 		int x = BAN_DisplayDamage(proc->anim2, 0, 0, 0, proc->roundId); 
	movs	r6, #73	@ tmp172,
@ C_Code.c:276: 		int x = BAN_DisplayDamage(proc->anim2, 0, 0, 0, proc->roundId); 
	...
	ldr	r5, .L359+20	@ tmp174,
	str	r3, [sp]	@ tmp173,
	movs	r3, #0	@,
	bl	.L102		@
	movs	r3, r0	@ x, tmp185
@ C_Code.c:277: 		x = BAN_DisplayDamage(proc->anim, 1, proc->anim->xPosition, x, proc->roundId);  
...
@ C_Code.c:280: } 
	...
	bx	r0
.L341:
@ C_Code.c:270: 	else if (EkrEfxIsUnitHittedNow(proc->side)) { proc->EkrEfxIsUnitHittedNowFrames = 0; } 
	movs	r3, #77	@ tmp153,
@ C_Code.c:270: 	else if (EkrEfxIsUnitHittedNow(proc->side)) { proc->EkrEfxIsUnitHittedNowFrames = 0; } 
	ldrb	r0, [r4, r3]	@ tmp154,
	ldr	r3, .L359+24	@ tmp155,
	bl	.L85		@
@ C_Code.c:270: 	else if (EkrEfxIsUnitHittedNow(proc->side)) { proc->EkrEfxIsUnitHittedNowFrames = 0; } 
	cmp	r0, #0	@ tmp182,
	beq	.L342		@,
...
GetBattleAnimPreconfType:
	...
	@ args = 0, pretend = 0, frame = 0
	@ frame_needed = 0, uses_anonymous_args = 0
@ C_Code.c:720: 	int result = gPlaySt.config.animationType; 
	movs	r2, #66	@ tmp130,
@ C_Code.c:719: int GetBattleAnimPreconfType(void) {
	push	{r4, lr}	@
@ C_Code.c:720: 	int result = gPlaySt.config.animationType; 
	ldr	r3, .L380	@ tmp164,
	ldrb	r0, [r3, r2]	@ gPlaySt, gPlaySt
@ C_Code.c:721: 	if (!CheatCodeOn()) { 
...
@ C_Code.c:721: 	if (!CheatCodeOn()) { 
	cmp	r2, #127	@ tmp139,
	bhi	.L369		@,
@ C_Code.c:722: 		if (ForceAnimsOn) { if (result == 2) { return 2; } else { return 1; } } 
...
@ C_Code.c:743: }
	...
	bx	r1
.L369:
@ C_Code.c:725:     if (gPlaySt.config.animationType != PLAY_ANIMCONF_SOLO_ANIM)
	movs	r2, #66	@ tmp143,
...
@ C_Code.c:725:     if (gPlaySt.config.animationType != PLAY_ANIMCONF_SOLO_ANIM)
	...
	cmp	r3, #4	@ tmp150,
	bne	.L368		@,
@ C_Code.c:729:     if (UNIT_FACTION(&gBattleActor.unit) == FACTION_BLUE)
	movs	r1, #11	@ tmp154,
@ C_Code.c:730:         if (UNIT_FACTION(&gBattleTarget.unit) == FACTION_BLUE)
...
@ C_Code.c:729:     if (UNIT_FACTION(&gBattleActor.unit) == FACTION_BLUE)
	...
	beq	.L379		@,
@ C_Code.c:735:         if (UNIT_FACTION(&gBattleTarget.unit) != FACTION_BLUE)
	tst	r3, r4	@ tmp155, pretmp_25
	bne	.L372		@,
@ C_Code.c:742:         return GetSoloAnimPreconfType(&gBattleTarget.unit);
	movs	r0, r2	@, tmp152
.L379:
