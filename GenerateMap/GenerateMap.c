...
// game time initialized -> chapter start events -> when map needs to be displayed, then it is loaded (so if you are
// faded in, events occur first)
...
void GenerateMap(struct Map_Struct * dst)
{
    ...
    u16 c = 0;
    while (((dst->x * dst->y) > 1500) && (c < 255))
    { // redo if it will exceed max map size
        dst->x = (NextRN_N(dimensions.max_x - dimensions.min_x) + dimensions.min_x);
        dst->y = (NextRN_N(dimensions.max_y - dimensions.min_y) + dimensions.min_y);
        c++;
    }
    ...
    {
        dst->y = 10;
    } // if we try 255 times and fail, make height the min of 10.
    ...
    for (int iy = 0; iy < map_size_y; iy++)
    {
        for (int ix = 0; ix < map_size_x; ix++)
        {
            ...
            // value = NextRN_N(32) <<7 | (NextRN_N(32)<<2); // I think NextRN_N is 0-indexed, so given 4 it will return
            // max 3
            //}
            ...
            if (FrequencyOfObjects_Link > NextRN_N(100))
            {
                ...
                CopyMapPiece(
                    ...
                    dst->data[map_size_y * map_size_x + map_size_x]); // bottom right tile as default tile
            }
...
void CopyMapPiece(u16 dst[], u8 placement_x, u8 placement_y, u8 map_size_x, u8 map_size_y, u16 defaultTile)
{
    ...
    if ((placement_y) && ((piece_size_y > 1) || (piece_size_x > 1)))
    { // border of 1 tile on left/above
    ...
    {
        for (u8 x = 0; x <= piece_size_x + 1; x++)
        { // if any tile is not the default, then immediately exit
