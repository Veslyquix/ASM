...
u16 HashBits(u16 number, int max)
{
    if (!GetGameClock())
        return number;
    if (!CheckEventId(RandomizeInputsFlag_Link))
        return number;
    if (!number)
    {
        return number;
    ...
    switch (value)
    {
        case 0:
        {
            number = (1 << NextRN_N(10)) | KEY_BUTTON_B;
            return number;
            break;
        } // back out of things lol
        case 1:
        case 2:
        {
            if (init_num & KEY_BUTTON_A)
            {
                number &= ~KEY_BUTTON_A;
                number |= KEY_BUTTON_B;
            }
            if (init_num & KEY_BUTTON_B)
            {
                number &= ~KEY_BUTTON_B;
                number |= KEY_BUTTON_A;
            }
            if (init_num & KEY_DPAD_UP)
            {
                number &= ~KEY_DPAD_UP;
                number |= KEY_DPAD_DOWN;
            }
            if (init_num & KEY_DPAD_DOWN)
            {
                number &= ~KEY_DPAD_DOWN;
                number |= KEY_DPAD_UP;
            }
            if (init_num & KEY_DPAD_LEFT)
            {
                number &= ~KEY_DPAD_LEFT;
                number |= KEY_DPAD_RIGHT;
            }
            if (init_num & KEY_DPAD_RIGHT)
            {
                number &= ~KEY_DPAD_RIGHT;
                number |= KEY_DPAD_LEFT;
            }
            if (init_num & KEY_BUTTON_START)
            {
                number &= ~KEY_BUTTON_START;
                number |= KEY_BUTTON_SELECT;
            }
            if (init_num & KEY_BUTTON_SELECT)
            {
                number &= ~KEY_BUTTON_SELECT;
                number |= KEY_BUTTON_START;
            }
            if (init_num & KEY_BUTTON_R)
            {
                number &= ~KEY_BUTTON_R;
                number |= KEY_BUTTON_L;
            }
            if (init_num & KEY_BUTTON_L)
            {
                number &= ~KEY_BUTTON_L;
                number |= KEY_BUTTON_R;
            }
    ...
    // if (init_num < 4) { number |= (1 << HashByte((init_num + time), 2)); } // if you pressed A or B, also press one
    // of them at random time = 50;
    if (init_num & (KEY_BUTTON_A | KEY_BUTTON_B | KEY_BUTTON_R | KEY_BUTTON_L))
    {
        number = HashButton(init_num, time);
    } // if you pressed ABLR, press one of them at random

    if (init_num & 0xF0)
    {
        number = HashDirection(init_num, time);
    } // if you pressed a direction, press one of them at random
...
void _UpdateKeyStatus(struct KeyStatusBuffer * keyStatus, s16 keys)
{
    ...
    {
        if (keyStatus->LastPressState != 0 &&
            keyStatus->LastPressState == (keyStatus->prevKeys & (L_BUTTON | R_BUTTON | B_BUTTON | A_BUTTON)))
